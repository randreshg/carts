...
static cl::opt<bool>
    GpuEnabled("gpu", cl::desc("Enable GPU compilation pipeline support"),
               cl::init(false));

static cl::opt<std::string>
    GpuArch("gpu-arch", cl::desc("GPU target architecture (e.g., sm_80)"),
            cl::value_desc("arch"), cl::init(""));
...
void setupOpenMPToArts(PassManager &pm, bool gpuEnabled, StringRef configPath) {
  pm.addPass(arts::createConvertOpenMPtoArtsPass(gpuEnabled));
  if (gpuEnabled)
    pm.addPass(arts::createGpuEligibilityAnalysisPass(configPath));
  pm.addPass(arts::createDeadCodeEliminationPass());
  pm.addPass(createCSEPass());
}
...
void setupEdtDistribution(PassManager &pm, arts::ArtsAnalysisManager *AM,
                          bool gpuEnabled, StringRef configPath) {
  pm.addPass(arts::createEdtDistributionPass(AM));
  if (gpuEnabled)
    pm.addPass(arts::createGpuForLoweringPass(configPath));
...
void setupPreLowering(PassManager &pm, arts::ArtsAnalysisManager *AM,
  ...
  pm.addPass(createCSEPass());
  if (gpuEnabled)
    pm.addPass(arts::createGpuEdtLoweringPass());
  pm.addPass(arts::createDbLoweringPass(ArtsIdStride));
  pm.addPass(polygeist::createPolygeistCanonicalizePass());
  pm.addPass(createCSEPass());
  pm.addPass(arts::createEdtLoweringPass(ArtsIdStride, gpuEnabled));
  pm.addPass(polygeist::createPolygeistCanonicalizePass());
  pm.addPass(createCSEPass());
...
setupPassManager(ModuleOp module, MLIRContext &context,
  ...
  if (GpuEnabled && !GpuArch.empty()) {
    module->setAttr("arts.gpu_arch", StringAttr::get(&context, GpuArch));
  }
  ...
  std::vector<StageSpec> stages = {
      ...
      {PipelineStage::OpenMPToArts, "Error when converting OpenMP to ARTS",
       [&](PassManager &pm) { setupOpenMPToArts(pm, GpuEnabled, ArtsConfig); }},
      ...
      {PipelineStage::EdtDistribution, "Error when running EDT distribution",
       [&](PassManager &pm) {
         setupEdtDistribution(pm, AM.get(), GpuEnabled, ArtsConfig);
       }},
      ...
      {PipelineStage::PreLowering,
       ...
       [&](PassManager &pm) { setupPreLowering(pm, AM.get(), GpuEnabled); }},
