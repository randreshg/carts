...
MAKE_VAR_EXTRA_CMAKE_FLAGS = "EXTRA_CMAKE_FLAGS"
...
def configured_make_vars(config: Any) -> list[str]:
    ...
    if extra_cmake_flags := getattr(config, "extra_cmake_flags", None):
        make_vars.append(f"{MAKE_VAR_EXTRA_CMAKE_FLAGS}={' '.join(extra_cmake_flags)}")
    return make_vars


def build_process_env(config: Any) -> dict[str, str]:
    """Return subprocess env for local make/cmake builds.

    macOS additionally needs ``SDKROOT`` for the SDK sysroot and the managed
    runtime library path so freshly built tools resolve the self-built C++
    runtime during the build.
    """
    env = dict(os.environ)
    env.update(build_parallel_env())

    if getattr(getattr(config, "info", None), "is_macos", False):
        if sdk_path := getattr(config, "macos_sdk_path", None):
            env["SDKROOT"] = str(sdk_path)
        runtime_paths = managed_runtime_library_env(
            config.carts_dir,
            include_existing_env=True,
            platform_info=config.info,
        )
        if runtime_paths:
            env["DYLD_LIBRARY_PATH"] = runtime_paths
