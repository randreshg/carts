...
class CartsConfig:
    ...
    def detect(cls, script_dir: Path | None = None) -> CartsConfig:
        ...
        config._setup_toolchain_build_flags()
    ...
    def _setup_paths(self) -> None:
        ...
        self.macos_sdk_path = self._detect_macos_sdk_path()

    @staticmethod
    def _detect_macos_sdk_path() -> Path | None:
        import subprocess
    ...
    def _setup_toolchain_build_flags(self) -> None:
        """Per-OS cmake flags passed to the Makefile as EXTRA_CMAKE_FLAGS.

        macOS pins the SDK sysroot, the self-built C++ runtime, and an install
        rpath; Linux/Windows need nothing extra.
        """
        if not self.info.is_macos:
            return

        runtime_flags = "-stdlib=libc++ -rtlib=compiler-rt --unwindlib=libunwind"
        install_lib = self.llvm_install_dir / "lib"
        linker_flags = f"-Wl,-rpath,{install_lib}"
        if self.linker_path:
            linker_flags = f"--ld-path={self.linker_path} {linker_flags}"

        flags = [f'-DCMAKE_CXX_FLAGS="{runtime_flags}"']
        if self.macos_sdk_path:
            flags.append(f"-DCMAKE_OSX_SYSROOT={self.macos_sdk_path}")
        flags.extend([
            f'-DCMAKE_EXE_LINKER_FLAGS="{linker_flags}"',
            f'-DCMAKE_SHARED_LINKER_FLAGS="{linker_flags}"',
            f'-DCMAKE_MODULE_LINKER_FLAGS="{linker_flags}"',
            f'-DCMAKE_INSTALL_RPATH="@loader_path/../lib;{install_lib}"',
            "-DCMAKE_BUILD_WITH_INSTALL_RPATH=ON",
            "-DCMAKE_INSTALL_RPATH_USE_LINK_PATH=ON",
        ])
        self.extra_cmake_flags = flags
