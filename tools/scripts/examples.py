...
# Optional per-example file of extra `carts compile` flags (whitespace/newline
# separated, `#` comments allowed).
COMPILE_FLAGS_FILENAME = "carts.flags"
...
def _read_compile_flags(example_dir: Path) -> List[str]:
    """Read extra compile flags from the example's carts.flags file, if any."""
    flags_file = example_dir / COMPILE_FLAGS_FILENAME
