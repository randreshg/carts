...
# Import shared styles for console output
...
def parse_and_aggregate_counters(counters_dir: Path) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    """
    aggregated_results_map: Dict[str, Dict[str, Union[int, float]]] = defaultdict(
        lambda: {"count": 0, "total_time": 0.0})
    raw_file_data_list: List[Dict[str, Any]] = []

    if not counters_dir.exists() or not counters_dir.is_dir():
        logger.warning(
            f"Counters directory {counters_dir} not found. Cannot parse counters.")
        return [], []
    ...
    logger.info(
        f"Parsing {len(ct_files)} counter files from {counters_dir}...")
    for ct_file_path in ct_files:
        ...
        try:
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    if is_header:
                        if line.startswith("name nodeId threadId count totalTime"):
                            ...
                            logger.warning(
                                f"File {ct_file_path} does not start with expected header. Attempting parse from line 1.")
                            is_header = False
                    ...
                    if len(parts) == 5:
                        try:
                            ...
                            # For aggregation
                            # type: ignore
                            aggregated_results_map[name_val]["count"] += count_val
                            # type: ignore
                            aggregated_results_map[name_val]["total_time"] += total_time_val
                            ...
                            logger.warning(
                                f"Skipping malformed data line {line_num} in {ct_file_path}: '{line}' (numeric conversion error)")
                        except Exception as e:
                            logger.warning(
                                f"Error processing data line {line_num} in {ct_file_path}: '{line}' - {e}")
                    else:
                        logger.warning(
                            f"Skipping line {line_num} in {ct_file_path} (unexpected columns {len(parts)}/5): '{line}'")
            ...
            logger.error(
                f"Could not read or process counter file {ct_file_path}: {e}")

    aggregated_output_list = [
        {"name": name, "aggregated_count": data["count"],
            "aggregated_total_time": data["total_time"]}
        for name, data in aggregated_results_map.items()
    ]
    logger.info(
        f"Aggregated {len(aggregated_output_list)} unique counters. Collected raw data from {len(raw_file_data_list)} files.")
...
class SystemInfoManager:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        if self._system_info is None:
            ...
            try:
                ...
                cpu_cores = re.search(
                    r"^CPU\(s\):\s*(\d+)", cpu_info_raw, re.MULTILINE)
                info["cpu_model"] = cpu_model.group(
                    1).strip() if cpu_model else "N/A"
                info["cpu_cores"] = int(
                    cpu_cores.group(1)) if cpu_cores else "N/A"
            except Exception as e:
    ...
    def get_reproducibility_info(self, project_root: Path) -> Dict[str, Any]:
        if self._reproducibility_info is None:
            ...
            info['env_vars'] = {k: v for k, v in os.environ.items(
            ) if k.startswith(('OMP_', 'CARTS_', 'PATH', 'LD_'))}
            info['git_commit_main_project'] = self._get_git_commit(
                project_root)
...
class DirectoryManager:
    ...
    def __init__(self, project_root: Path, example_base_dirs_config: List[str], output_dir_name: str = "output"):
        ...
        self.example_base_paths = [project_root /
                                   d for d in example_base_dirs_config]
    ...
    def setup_output_directory(self, clear_existing: bool) -> Path:
        if clear_existing:
            ...
            self.output_dir = self._get_next_available_dir(
                self.base_output_dir)

        self.output_dir.mkdir(parents=True, exist_ok=True)
        logger.info(
            f"Output will be saved to directory: {self.output_dir.resolve()}")
    ...
    def find_example_directories(self, target_example_names: Optional[List[str]] = None) -> List[Path]:
        ...
        for base_path in self.example_base_paths:
            if not base_path.is_dir():
                logger.warning(
                    f"Example base path {base_path} does not exist or is not a directory.")
                continue
        ...
        logger.info(
            f"Found {len(example_dirs)} example directories with test_config.yaml from bases {self.example_base_paths}.")

        if target_example_names:
            filtered_dirs = [
                d for d in example_dirs if d.name in target_example_names]
            if len(filtered_dirs) != len(target_example_names):
                found_names = {d.name for d in filtered_dirs}
                missing_targets = [
                    t for t in target_example_names if t not in found_names]
                logger.warning(
                    f"Could not find all specified target examples. Missing: {missing_targets}")
            example_dirs = filtered_dirs
            logger.info(
                f"Filtered to {len(example_dirs)} target examples: {[d.name for d in example_dirs]}")
...
class ConfigManager:
    ...
    def __init__(self, example_dir: Path, cli_args: Optional[argparse.Namespace] = None):
        ...
        self._arts_config_cache: Optional[Dict[str,
    ...
    def load_test_config(self) -> Dict[str, Any]:
        if self._test_config is None:
            if not self.test_config_path.is_file():
                logger.error(
                    f"test_config.yaml not found in {self.example_dir}")
                return {}
            try:
                ...
                logger.error(
                    f"Unexpected error loading {self.test_config_path}: {e}")
                return {}
    ...
    def get_run_plan(self) -> List[Tuple[str, int, str]]:
        ...
        if not isinstance(problem_sizes, list) or not isinstance(iterations_list, list) or not isinstance(args_list, list):
            logger.error(
                f"problem_sizes, iterations, or args is not a list in {self.test_config_path}")
            return []
        if not (len(problem_sizes) == len(iterations_list) == len(args_list)):
            logger.error(
                f"Mismatched lengths for problem_sizes, iterations, or args in {self.test_config_path}")
            return []
    ...
    def parse_arts_cfg(self) -> Dict[str, Optional[Union[int, str]]]:
        config: Dict[str, Optional[Union[int, str]]] = {
            "threads": None, "nodeCount": None}
                ...
                for line in f:
                    ...
                    if key == "threads":
                        try:
                            ...
                            logger.warning(
                                f"Non-integer 'threads' in {self.arts_cfg_path}: {value}")
                    elif key == "nodeCount":
                        try:
                            config["nodeCount"] = int(value)
                        except ValueError:
                            logger.warning(
                                f"Non-integer 'nodeCount' in {self.arts_cfg_path}: {value}")
        except Exception as e:
    ...
    def update_arts_cfg_threads(self, num_threads: int) -> None:
        ...
        try:
            with open(self.arts_cfg_path, 'w') as f:
                f.writelines(lines)
            logger.debug(
                f"Updated 'threads = {num_threads}' in {self.arts_cfg_path}")
        except Exception as e:
    ...
    def configure_arts_for_introspection(self, enable: bool):
        ...
        if not self.arts_cfg_path.exists() and enable:
            logger.warning(
                f"arts.cfg {self.arts_cfg_path} not found. Creating for introspection.")
            with open(self.arts_cfg_path, 'w') as f:
                f.write("counterFolder = ./counters\n")
                f.write("counterStartPoint = 1\n")
            logger.debug(
                f"Created and enabled introspection in {self.arts_cfg_path}")
            return
        elif not self.arts_cfg_path.exists():
            logger.debug(
                f"arts.cfg {self.arts_cfg_path} not found. No action for disabling introspection.")
            return
        ...
        lines = [line for line in lines if not line.strip().startswith(
            ("counterFolder", "counterStartPoint"))]

        if enable:
            lines.append("counterFolder = ./counters\n")
            lines.append("counterStartPoint = 1\n")
            logger.debug(
                f"Enabled introspection settings in {self.arts_cfg_path}")
        else:
            if len(lines) < original_lines_count:
                logger.debug(
                    f"Disabled introspection settings in {self.arts_cfg_path}")
            else:
                logger.debug(
                    f"Introspection settings already disabled/absent in {self.arts_cfg_path}")
    ...
    def get_grouped_perf_events(self) -> Optional[List[List[str]]]:
        ...
        if isinstance(yaml_perf_events, list):
            ...
            logger.warning(
                f"perf_events in {self.test_config_path} should be a list. Ignoring.")

        def ensure_user_mode(ev: str) -> str:
            return ev if ev.endswith((":u", ":k")) else ev + ":u"

        all_defined_events = DEFAULT_PERF_EVENTS + \
            [ensure_user_mode(e) for e in extra_events]
        ...
        for _, group_config_events in EVENT_GROUPS.items():
            current_group_run = [
                ev for ev in group_config_events if ev in unique_events]
            if current_group_run:
                grouped_runs.append(current_group_run)
                used_events.update(current_group_run)
        remaining_events = [
            ev for ev in unique_events if ev not in used_events]
        if remaining_events:
...
class ExecutionManager:
    ...
    def run_make_command(self, make_target: str, cwd: Path) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error during 'make {make_target}' in {cwd}: {e.stderr}")
            return False
            ...
            logger.error(
                f"TIMEOUT during 'make {make_target}' in {cwd} after {self.timeout_seconds * 2}s.")
            return False
    ...
    def _run_single_program(self, command_list: List[str], cwd: Path, iteration_num: int, run_description: str = "") -> Tuple[Optional[float], str, str]:
        ...
        logger.debug(
            f"Executing (Iter {iteration_num + 1}{' - ' + run_description if run_description else ''}): {cmd_str} in {cwd}")
        try:
            process = subprocess.run(
            ...
            time_match = re.search(
                r"Finished in\s+([0-9]*\.?[0-9]+)\s*seconds?", full_output, re.IGNORECASE)
            if time_match:
                time_val = float(time_match.group(1))
            else:
                time_match_perf = re.search(
                    r"([0-9]*\.?[0-9]+)\s+seconds time elapsed", full_output, re.IGNORECASE)
                if time_match_perf:
                    time_val = float(time_match_perf.group(1))
                else:
                    logger.warning(
                        f"Iter {iteration_num + 1}{' - ' + run_description if run_description else ''}: Time not parsed. Output: {full_output[:200].replace(chr(10), ' ')}...")

            correctness_match = re.search(
                r"Result: (CORRECT|INCORRECT)", full_output, re.IGNORECASE)
            if correctness_match:
                correctness = correctness_match.group(1).upper()
            elif re.search(r"Result: OK", full_output, re.IGNORECASE):
                correctness = "CORRECT"
        except subprocess.TimeoutExpired:
            logger.warning(
                f"Iter {iteration_num + 1}{' - ' + run_description if run_description else ''}: TIMEOUT for {command_list[0]}.")
            correctness = "TIMEOUT"
            full_output += f"\nTIMEOUT after {self.timeout_seconds}s."
        except subprocess.CalledProcessError as e:
            logger.error(
                f"Error running {command_list[0]} (iter {iteration_num + 1}{' - ' + run_description if run_description else ''}): {e.stderr or e.stdout}")
            full_output = (e.stdout or "") + "\n" + (e.stderr or "")
            correctness = "ERROR_RUNTIME"
        except FileNotFoundError:
            logger.error(f"Executable {command_list[0]} not found.")
            correctness = "ERROR_MISSING_EXE"
            full_output = f"Executable {command_list[0]} not found."
        except Exception as e:
            logger.error(
                f"Unexpected error running {command_list[0]} (iter {iteration_num + 1}{' - ' + run_description if run_description else ''}): {e}")
            correctness = "ERROR_UNEXPECTED"
            full_output += f"\nUnexpected error: {e}"
    ...
    def run_benchmark_instance(self, executable_path: Path, program_args_str: str, num_iterations: int,
        ...
        if not executable_path.exists():
            logger.error(
                f"Executable {executable_path} not found for {version_name} {example_name}. Skipping benchmark.")
        ...
        logger.info(
            f"Running benchmark: {version_name} ('{executable_path.name}') for '{example_name}', args='{program_args_str}', iters={num_iterations}...")
        times: List[Optional[float]] = []
        final_correctness = "UNKNOWN"
        all_outputs: List[str] = []
        command = [str(executable_path)] + program_args_str.split()
        for i in range(num_iterations):
            time_val, iter_correctness, iter_output = self._run_single_program(
                command, executable_path.parent, i, f"{version_name} benchmark")
            ...
            logger.debug(
                f"  Iter {i+1} ({version_name} benchmark): {safe_fmt(time_val, '.6f')}s, Status: {iter_correctness}")
    ...
    def run_profiling_instance(self, executable_path: Path, program_args_str: str, num_iterations: int,
        ...
        if not executable_path.exists():
            logger.error(
                f"Executable {executable_path} not found for {version_name} {example_name}. Skipping profiling.")
        ...
        logger.info(
            f"Running profiling: {version_name} ('{executable_path.name}') for '{example_name}', args='{program_args_str}', iters={num_iterations}...")
        event_data_accumulator: Dict[str, List[float]] = defaultdict(list)
        times_from_perf_runs: List[Optional[float]] = []
        final_correctness = "UNKNOWN"
        all_outputs_profiling: List[str] = []
        program_arg_list = program_args_str.split()
        for i in range(num_iterations):
            iter_correctness_overall = "UNKNOWN"
            time_for_this_iter: Optional[float] = None
            iter_output_combined_groups = ""
            for group_idx, event_group in enumerate(perf_event_groups):
                if not event_group:
                    continue
                perf_cmd = ["perf", "stat", "-a", "-x", ",", "-e",
                            ','.join(event_group), "--", str(executable_path)] + program_arg_list
                run_desc = f"{version_name} profiling group {group_idx+1}/{len(perf_event_groups)}"
                time_val, iter_correctness_group, perf_output_single_group = self._run_single_program(
                    perf_cmd, executable_path.parent, i, run_desc)
                ...
                for line in perf_output_single_group.splitlines():
                    ...
                    if len(parts) >= 3 and not parts[0].strip().startswith(("#", "<not counted>", "<not supported>")):
                        try:
                            ...
                            val = float('nan') if val_str.lower() == 'nan' or val_str in [
                                '<not supported>', '<not counted>'] else float(val_str)
                            event_data_accumulator[parts[2].strip()].append(
                                val)
                        except (ValueError, IndexError):
                            pass
            all_outputs_profiling.append(iter_output_combined_groups)
            ...
            logger.debug(
                f"  Iter {i+1} ({version_name} profiling): {safe_fmt(time_for_this_iter, '.6f')}s, Status: {iter_correctness_overall}")
        processed_event_stats = {event: {"all_values": [v for v in values if not math.isnan(
            v)]} for event, values in event_data_accumulator.items()}
    ...
    def run_arts_inspector(self, inspector_path: Path, program_args_str: str, cwd: Path) -> bool:
        if not inspector_path.exists():
            logger.error(
                f"ARTS Inspector executable {inspector_path} not found. Skipping.")
            return False
        logger.info(
            f"Running ARTS Inspector: {inspector_path.name} with args '{program_args_str}' in {cwd}")
        command = [str(inspector_path)] + program_args_str.split()
        _time_val, status, output = self._run_single_program(
            command, cwd, 0, "ARTS Inspector")
        if status not in ["CORRECT", "OK", "UNKNOWN", "ERROR_RUNTIME"]:
            if status in ["TIMEOUT", "ERROR_MISSING_EXE", "ERROR_UNEXPECTED"]:
                logger.error(
                    f"ARTS Inspector critical fail: {status}. Output: {output[:500]}")
                return False
            else:
                logger.warning(
                    f"ARTS Inspector completed with status: {status}. Output: {output[:500]}")
                return True
...
class ResultsManager:
    ...
    def prepare_aggregated_data(self) -> List[Dict[str, Any]]:
        def key_func(r): return (r["example_name"],
                                 r["problem_size"], r.get("threads", 1))
        ...
        for run in self._benchmark_runs:
            key = key_func(run)
            version = run["version"].upper()
            processed_runs[key][f"{version}_benchmark_data"] = run
            if version == "CARTS" and run.get("carts_config"):
                processed_runs[key]["carts_config_details"] = run.get(
                    "carts_config", {})
        ...
        for key_tuple, data_group in sorted(processed_runs.items()):
            ex_name, prob_size, threads = key_tuple
            carts_cfg = data_group.get("carts_config_details", {})
            ...
            carts_b = data_group.get("CARTS_benchmark_data")
            omp_b = data_group.get("OMP_benchmark_data")
            carts_t = carts_b.get("times_seconds", []) if carts_b else []
            omp_t = omp_b.get("times_seconds", []) if omp_b else []
            carts_stats = self._compute_runtime_stats(carts_t)
            omp_stats = self._compute_runtime_stats(omp_t)

            row.update({"CARTS Median Time (s)": carts_stats["median"], "CARTS Avg Time (s)": carts_stats["avg"], "CARTS Min Time (s)": carts_stats["min"], "CARTS Max Time (s)": carts_stats["max"], "CARTS StdDev Time (s)": carts_stats["stdev"], "CARTS Times (s)": carts_t, "CARTS Correctness": carts_b.get(
                "correctness_status", "N/A") if carts_b else "N/A", "CARTS Iterations Ran": carts_b.get("iterations_ran", 0) if carts_b else 0, "CARTS Iterations Requested": carts_b.get("iterations_requested", 0) if carts_b else 0})
            row.update({"OMP Median Time (s)": omp_stats["median"], "OMP Avg Time (s)": omp_stats["avg"], "OMP Min Time (s)": omp_stats["min"], "OMP Max Time (s)": omp_stats["max"], "OMP StdDev Time (s)": omp_stats["stdev"], "OMP Times (s)": omp_t, "OMP Correctness": omp_b.get(
                "correctness_status", "N/A") if omp_b else "N/A", "OMP Iterations Ran": omp_b.get("iterations_ran", 0) if omp_b else 0, "OMP Iterations Requested": omp_b.get("iterations_requested", 0) if omp_b else 0})

            row["Speedup (OMP/CARTS)"] = (omp_stats["median"] / carts_stats["median"]
                                          ) if carts_stats["median"] and omp_stats["median"] and carts_stats["median"] > 1e-9 else None

            for ver_prefix in ["CARTS", "OMP"]:
                prof_run = data_group.get(f"{ver_prefix}_profiling_data")
                events_raw = prof_run.get(
                    "event_stats", {}) if prof_run else {}
    ...
    def compute_overall_statistics(self, aggregated_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        speedups = [r["Speedup (OMP/CARTS)"] for r in aggregated_data if r.get("Speedup (OMP/CARTS)") is not None and not (
            isinstance(r["Speedup (OMP/CARTS)"], float) and math.isnan(r["Speedup (OMP/CARTS)"]))]
        ...
        for r in aggregated_data:
            ...
            if s is not None and not (isinstance(s, float) and math.isnan(s)):
                ...
                if s > 1.05:
                    ...
                    ties += 1
                if max_s_val is None or s > max_s_val:
                    max_s_val, max_s_ex = s, cfg_str
                if s < (1/1.05) and (max_slow_val is None or s < max_slow_val):
                    # s is small (CARTS slower)
                    max_slow_val, max_slow_ex = s, cfg_str
                if s > 0 and (min_s_val is None or s < min_s_val):
                    min_s_val, min_s_ex = s, cfg_str

        geom_mean = None
        valid_s_geom = [s for s in speedups if s > 0]
        if valid_s_geom:
            try:
                geom_mean = statistics.geometric_mean(valid_s_geom)
            except statistics.StatisticsError:
                logger.warning("Geom. mean error for speedups.")

        correctness = defaultdict(int)
        for run in self._benchmark_runs:
            correctness[f"{run['version'].upper()}_{run.get('correctness_status', 'UNKNOWN')}"] += 1
    ...
    def collect_failure_report(self, aggregated_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        failures = []
        fail_stati = ["INCORRECT", "TIMEOUT", "ERROR_RUNTIME",
                      "ERROR_MISSING_EXE", "ERROR_UNEXPECTED"]
        for r in aggregated_data:
    ...
    def write_csv_report(self, aggregated_data: List[Dict[str, Any]], output_path: Path) -> None:
        if not aggregated_data:
            logger.info("No aggregated data for CSV.")
            return
        fnames = list(aggregated_data[0].keys())
        common = ["Example Name", "Problem Size", "Threads", "CARTS Config Actual Threads", "CARTS Config Actual Nodes",
                  "Speedup (OMP/CARTS)", "CARTS Median Time (s)", "OMP Median Time (s)", "CARTS Correctness", "OMP Correctness"]
        ordered_fnames = [f for f in common if f in fnames] + \
            sorted([f for f in fnames if f not in common])
        with open(output_path, 'w', newline='') as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=ordered_fnames, restval="N/A", extrasaction='ignore')
            writer.writeheader()
            for row in aggregated_data:
                ...
                for k, v in row.items():
                    if isinstance(v, float):
                        fmt_row[k] = safe_fmt(
                            v, ".6f" if "Time (s)" in k or "Speedup" in k else ".2f")
                    elif k.endswith("Times (s)") and isinstance(v, list):
                        fmt_row[k] = "; ".join([safe_fmt(t, ".6f") for t in v])
                    elif isinstance(v, list):
                        fmt_row[k] = "; ".join([str(x) for x in v])
                    else:
                        fmt_row[k] = v if v is not None else "N/A"
    ...
    def write_json_report(self, final_json_data: Dict[str, Any], output_path: Path) -> None:
        def default_serializer(obj):
            ...
            raise TypeError(
                f"Object of type {obj.__class__.__name__} is not JSON serializable: {obj}")
    ...
    def save_figure(self, fig, img_path: Path, pdf_path: Path):
        try:
            ...
            logger.info(f"Saved PNG to {img_path}")
...
def main():
    print_header("CARTS Performance and Introspection Script")
    logger.info("- START CARTS Performance and Introspection Script -")
    parser = argparse.ArgumentParser(
        description="Run CARTS examples, benchmarks, profiling, and introspection.")
    parser.add_argument("--timeout_seconds", type=int,
                        default=300, help="Timeout for each program run.")
    parser.add_argument("--output_prefix", type=str,
                        default="performance_results", help="Prefix for output files.")
    parser.add_argument("--example_base_dirs", nargs='+', default=[
                        "examples/parallel", "examples/tasking"], help="Base example dirs.")
    parser.add_argument("--target_examples", nargs='+',
                        default=None, help="Specific examples to run.")
    parser.add_argument('--clear_output', action=argparse.BooleanOptionalAction,
                        default=True, help='Clear/reuse output dir vs. new one.')
    parser.add_argument('--perf_events', nargs='+',
                        default=None, help='CLI override for perf events.')
    args = parser.parse_args()

    project_root = Path(__file__).resolve().parent
    dir_manager = DirectoryManager(
        project_root, args.example_base_dirs, "output_benchmarks")
    ...
    example_paths_to_run = dir_manager.find_example_directories(
        args.target_examples)
    if not example_paths_to_run:
        logger.info("No examples found. Exiting.")
        return
    run_id_counter = 1
    ...
    for example_path in example_paths_to_run:
        ...
        if not config_manager.load_test_config():
            logger.warning(
                f"Skipping {example_name}: invalid test_config.yaml.")
            continue
        ...
        if not run_plan:
            logger.warning(f"Skipping {example_name}: invalid run plan.")
            continue

        for common_threads in threads_list:
            logger.info(
                f"--- Config: {example_name}, Threads={common_threads} ---")
            ...
            if not exec_manager.run_make_command("clean", example_path) or \
               not exec_manager.run_make_command("all", example_path):
                logger.error(
                    f"Make failed for {example_name}, threads={common_threads}. Skipping this thread config.")
                continue
            ...
            for prob_size, num_iters, spec_args in run_plan:
                logger.info(
                    f"--- Run: Problem='{prob_size}', Threads={common_threads} ---")
                exec_map = {"CARTS": example_path / example_name,
                            "OMP": example_path / f"{example_name}_omp"}
                run_args = " ".join([arg for arg in prob_size.split(
                ) if arg] + [arg for arg in spec_args.split() if arg])

                for ver_name, exec_file in exec_map.items():
                    bench_data = exec_manager.run_benchmark_instance(
                        exec_file, run_args, num_iters, example_name, ver_name)
                    ...
                    if is_profiling_on and perf_groups:
                        prof_data = exec_manager.run_profiling_instance(
                            exec_file, run_args, num_iters, example_name, ver_name, perf_groups)
                ...
                if is_arts_introspection_on:
                    logger.info(
                        f"ARTS Introspection: {example_name}, Size: {prob_size}, Threads: {common_threads}")
                    inspector_exec = example_path / f"{example_name}_inspector"
                    if inspector_exec.is_file():
                        config_manager.configure_arts_for_introspection(
                            enable=True)
                        counters_dir = example_path / "counters"
                        if counters_dir.exists():
                            shutil.rmtree(counters_dir)
                        counters_dir.mkdir(parents=True, exist_ok=True)

                        inspector_ok = exec_manager.run_arts_inspector(
                            inspector_exec, run_args, example_path)
                        if inspector_ok:
                            # parse_and_aggregate_counters now returns two lists
                            aggregated_counters_list, raw_details_list = parse_and_aggregate_counters(
                                counters_dir)
                            if aggregated_counters_list or raw_details_list:  # Add if either has data
                                ...
                                logger.warning(
                                    f"No counter data for {example_name}, Size: {prob_size}, Threads: {common_threads}")
                        else:
                            logger.error(
                                f"ARTS Inspector failed for {example_name}, Size: {prob_size}, Threads: {common_threads}. No counters.")
                        config_manager.configure_arts_for_introspection(
                            enable=False)
                    else:
                        logger.warning(
                            f"ARTS inspector not found: {inspector_exec}. Skipping.")

    print_info("Aggregating and Reporting Results")
    logger.info("\n--- Aggregating and Reporting Results ---")
    aggregated_data = results_manager.prepare_aggregated_data()
    results_manager.write_csv_report(
        aggregated_data, output_dir / f"{args.output_prefix}_summary.csv")
    ...
    results_manager.write_json_report(
        final_json, output_dir / f"{args.output_prefix}_full_data.json")

    print_header("Overall Statistics Highlights")
    geom_mean = safe_fmt(overall_stats.get('geometric_mean_speedup'), '.3f')
    carts_faster = overall_stats.get('carts_faster_configs')
    omp_faster = overall_stats.get('omp_faster_configs')
    tied = overall_stats.get('tied_configs')
    print_info(f"Geom. Mean Speedup (OMP/CARTS): {geom_mean}")
    print_info(
        f"CARTS Faster: {carts_faster}, OMP Faster: {omp_faster}, Tied: {tied}")
    print_success(f"Script finished. Output in {output_dir.resolve()}")
    logger.info("\nOverall Statistics Highlights:")
    logger.info(f"  Geom. Mean Speedup (OMP/CARTS): {geom_mean}")
    logger.info(
        f"  CARTS Faster: {carts_faster}, OMP Faster: {omp_faster}, Tied: {tied}")
