...
static const AnalysisKind kDbPartitioning_reads[] = {
    ...
    AnalysisKind::LoopAnalysis, AnalysisKind::RuntimeConfig};
...
DbPartitioningPass::partitionAlloc(DbAllocOp allocOp, DbAllocNode *allocNode) {
  ...
  ctx.machine = &AM->getRuntimeConfig();
  ...
  bool isMultinodeMachine = AM->getRuntimeConfig().hasValidNodeCount() &&
  ...
  const RuntimeConfig *machine = &AM->getRuntimeConfig();
...
bool DbPartitioningPass::resolveBlockPlanSizes(
  ...
  if (AM) {
    RuntimeConfig &machine = AM->getRuntimeConfig();
