///==========================================================================///
/// SdeLoopInterchange.cpp - Cache-optimal loop interchange via linalg carriers
///
...
namespace mlir::arts {
#define GEN_PASS_DEF_SDELOOPINTERCHANGE
...
ARTS_DEBUG_SETUP(sde_loop_interchange);
...
static bool interchangeLoops(scf::ForOp jLoop, scf::ForOp kLoop,
  ...
  if (kLoop->getParentOp() != jLoop.getOperation()) {
    ARTS_DEBUG("SdeLoopInterchange: k loop is not directly inside j loop");
    return false;
  }
  ...
  if (!collectRematerializablePrefixOps(initOps, kLoop,
    ...
    ARTS_DEBUG("SdeLoopInterchange: reduction body captures side-effecting "
               "prefix ops");
    return false;
  }
...
struct SdeLoopInterchangePass
    : public arts::impl::SdeLoopInterchangeBase<SdeLoopInterchangePass> {
  ...
  bool tryCarrierInterchange(sde::SdeSuIterateOp op, Block &body,
    ...
    ARTS_DEBUG("SdeLoopInterchange: general N-dim interchange triggered");
    return interchangeLoops(jLoop, kLoop, initOps);
  }
  ...
  bool tryStencilHaloInterchange(sde::SdeSuIterateOp op, Block &body) {
    ...
    if (innerLoop->getParentOp() != outerLoop.getOperation()) {
      ARTS_DEBUG("SdeLoopInterchange: stencil inner loop not directly nested");
      return false;
    }
  ...
  void runOnOperation() override {
    ...
    ARTS_INFO_HEADER(SdeLoopInterchangePass);
    ...
    ARTS_DEBUG("SdeLoopInterchangePass: interchanged " << interchangeCount
                                                       << " loop nests");
  }
};
...
std::unique_ptr<Pass> createSdeLoopInterchangePass() {
  return std::make_unique<SdeLoopInterchangePass>();
}
