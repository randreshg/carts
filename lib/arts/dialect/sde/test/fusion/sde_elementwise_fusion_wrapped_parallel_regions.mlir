...
// Verify that ElementwiseFusion looks through single-loop cu_region <parallel>
// wrappers emitted for consecutive OpenMP regions and preserves one wrapper
// around the fused elementwise pipeline.

// CHECK-LABEL: // -----// IR Dump After ElementwiseFusion (elementwise-fusion) //----- //
// CHECK: func.func @main
// CHECK: arts_sde.cu_region <parallel> {
// CHECK-NEXT: arts_sde.su_iterate (%c0) to (%c128) step (%{{.+}}) nowait classification(<elementwise_pipeline>) {
// CHECK-NOT: classification(<elementwise>)
