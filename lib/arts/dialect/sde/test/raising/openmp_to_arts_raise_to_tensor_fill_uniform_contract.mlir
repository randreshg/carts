...
// Verify the SDE tensor carrier flow for a write-only elementwise loop:
// RaiseToLinalg materializes a transient tensor-backed linalg.generic directly
// (with bufferization.to_tensor inputs), RaiseToTensor is a no-op since carriers
// are already tensor-backed, and ConvertSdeToArts consumes the carrier chain so
// downstream ARTS passes keep seeing only loop/memref IR.
...
// LINALG: %[[OUT:.+]] = bufferization.to_tensor %arg0 : memref<128xf64> to tensor<128xf64>
...
// TENSOR: %[[OUT:.+]] = bufferization.to_tensor %arg0 : memref<128xf64> to tensor<128xf64>
