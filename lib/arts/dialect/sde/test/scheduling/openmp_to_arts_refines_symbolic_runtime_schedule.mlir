...
// Verify that a symbolic-trip runtime schedule is refined to static for scalar
// SDE loops. ChunkOpt keeps the refinement attribute-only; no chunk operand is
// added.
...
// CHECK: arts_sde.su_iterate (%c0) to (%[[N]]) step (%{{.+}}) schedule(<static>)
// CHECK-NOT: arts.for

// CHECK-LABEL: // -----// IR Dump After ChunkOpt (chunk-opt) //----- //
// CHECK: func.func @main(%[[N:.*]]: index, %[[A:.*]]: memref<?xf64>, %[[B:.*]]: memref<?xf64>)
// CHECK: arts_sde.su_iterate (%c0) to (%[[N]]) step (%{{.+}}) schedule(<static>)
// CHECK-NOT: arts_sde.su_iterate{{.*}}schedule(<runtime>,
