...
// Verify that a symbolic-trip auto schedule remains target-neutral after SDE
// chunk shaping when the concrete schedule cannot be proven stable at SDE time.
...
// CHECK: func.func @main(%[[N:.*]]: index, %[[A:.*]]: memref<?xf64>, %[[B:.*]]: memref<?xf64>)
// CHECK: sde.cu_region <parallel> {
// CHECK: sde.su_iterate (%c0) to (%[[N]]) step (%{{.+}}) schedule(<auto>)
// CHECK-NOT: arts.for
