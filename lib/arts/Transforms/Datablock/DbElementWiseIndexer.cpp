...
DbElementWiseIndexer::localizeLinearized(Value globalLinearIndex, Value stride,
  ...
  ARTS_DEBUG("DbElementWiseIndexer::localizeLinearized - scaling offset by stride");
...
void DbElementWiseIndexer::transformOps(
    ...
    llvm::SetVector<Operation *> &opsToRemove) {
  ARTS_DEBUG("DbElementWiseIndexer::transformOps with " << ops.size() << " ops");
    ...
    if (outerRank_ == 1 && elementIndices.size() == 1 &&
      ...
      if (stride) {
        if (auto staticStride =
          ...
          if (*staticStride > 1) {
            isLinearized = true;
            ARTS_DEBUG("transformDbRefUsers: linearized stride=" << *staticStride);
          }
        } else {
          /// Dynamic stride - always use it
          isLinearized = true;
          ARTS_DEBUG("transformDbRefUsers: using dynamic stride from oldElementSizes");
        }
      }
    }
  ...
  if (globalIndices.size() == 1 && !sliceOffsets.empty()) {
    ...
    if (oldElementSizes_.size() >= 2) {
      ...
      if (stride) {
        ...
        if (auto staticStride =
                DatablockUtils::getStaticStride(oldElementSizes_)) {
          ARTS_DEBUG("  localizeCoordinates: LINEARIZED, stride=" << *staticStride);
        } else {
...
void DbElementWiseIndexer::transformUsesInParentRegion(
  ...
  for (Operation *op : users)
    transformAccess(op, dbAlloc.getPtr(), elementMemRefType, AC,
                               opsToRemove);
}
