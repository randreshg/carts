...
class DbCoarsePartitionPlanner final : public DbPartitionPlanner {
public:
  void computeAllocationShape(
      ...
      SmallVector<Value> &newInnerSizes) const override {
  ...
  void buildRewriteAcquire(const DbAcquirePartitionView &input,
...
class DbElementWisePartitionPlanner final : public DbPartitionPlanner {
public:
  void computeAllocationShape(
      DbAllocOp allocOp, const PartitioningDecision &decision,
      ArrayRef<Value> blockSizes, ArrayRef<unsigned> partitionedDims,
...
class DbBlockPartitionPlanner : public DbPartitionPlanner {
public:
  void computeAllocationShape(
      DbAllocOp allocOp, const PartitioningDecision &decision,
      ArrayRef<Value> blockSizes, ArrayRef<unsigned> partitionedDims,
...
class DbStencilPartitionPlanner final : public DbBlockPartitionPlanner {
public:
  void buildRewriteAcquire(const DbAcquirePartitionView &input,
                           DbAllocOp allocOp, const DbRewritePlan &plan,
                           DbRewriteAcquire &output,
                           OpBuilder &builder) const override {
    DbBlockPartitionPlanner::buildRewriteAcquire(input, allocOp, plan, output,
                                                 builder);
...
std::unique_ptr<DbPartitionPlanner>
DbPartitionPlanner::create(PartitionMode mode) {
  switch (mode) {
  case PartitionMode::coarse:
    return std::make_unique<DbCoarsePartitionPlanner>();
  case PartitionMode::fine_grained:
    return std::make_unique<DbElementWisePartitionPlanner>();
  case PartitionMode::block:
    return std::make_unique<DbBlockPartitionPlanner>();
  case PartitionMode::stencil:
    return std::make_unique<DbStencilPartitionPlanner>();
  }
  return std::make_unique<DbCoarsePartitionPlanner>();
}
