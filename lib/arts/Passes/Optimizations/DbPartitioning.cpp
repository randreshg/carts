...
struct AcquirePartitionInfo {
  ...
  bool isValid = false;
  /// For mixed mode: this coarse acquire needs full-range access to chunked alloc.
  ...
  case PartitionMode::Chunked:
    ...
    if (acqNode) {
      ...
      if (succeeded(acqNode->computeChunkInfo(offset, size))) {
        ...
        /// Validation failed, but trust hints directly if they exist.
        /// Hints are generated by ForLowering from parallel loop analysis
        /// and should be reliable even if conservative validation fails.
        auto hints = acquire.getOffsetHints();
        auto sizeHints = acquire.getSizeHints();
        if (!hints.empty() && !sizeHints.empty()) {
          info.partitionOffset = hints.front();
          info.partitionSize = sizeHints.front();
          info.isValid = true;
          ARTS_DEBUG("  Using offset/size hints directly (validation relaxed)");
        } else {
...
/// Return true if value depends on offset through arithmetic ops.
static bool dependsOnOffset(Value value, Value offset) {
  if (!value || !offset)
    return false;
...
static Value traceBackChunkSize(Value chunkSize, DbAllocOp allocOp,
  ...
  auto isZeroValue = [](Value value) -> bool {
    if (!value)
      return false;
    Value stripped = ValueUtils::stripNumericCasts(value);
  ...
  if (auto selectOp = dyn_cast<arith::SelectOp>(defOp)) {
    ...
    if (trueTraced && falseTraced) {
      if (isZeroValue(trueTraced) || isZeroValue(falseTraced)) {
        Value nonZero =
            isZeroValue(trueTraced) ? falseTraced : trueTraced;
        if (isZeroValue(nonZero))
          return nullptr;
        ARTS_DEBUG("  Select includes zero arm - using non-zero arm");
        return nonZero;
      }
    ...
    if (trueTraced && !falseTraced) {
      ...
      return isZeroValue(trueTraced) ? Value() : trueTraced;
    }
    if (falseTraced && !trueTraced) {
      ARTS_DEBUG("  Using false arm of select as max chunk size");
      return isZeroValue(falseTraced) ? Value() : falseTraced;
    }
  }
  ...
  if (auto minOp = dyn_cast<arith::MinSIOp>(defOp)) {
    ...
    Value rhsTraced = getTracedOrDominatingOperand(rhs);
    if (lhsTraced && rhsTraced) {
      if (isZeroValue(lhsTraced) || isZeroValue(rhsTraced)) {
        Value nonZero = isZeroValue(lhsTraced) ? rhsTraced : lhsTraced;
        if (isZeroValue(nonZero))
          return nullptr;
        ARTS_DEBUG("  MinSI includes zero operand - using non-zero operand");
        return nonZero;
      }
    ...
    if (rhsTraced && !lhsTraced) {
      ...
      return isZeroValue(rhsTraced) ? Value() : rhsTraced;
    }
    if (lhsTraced && !rhsTraced) {
      ARTS_DEBUG("  Using lhs of minsi as max chunk size");
      return isZeroValue(lhsTraced) ? Value() : lhsTraced;
    }
  }

  /// Pattern 7.5: arith.minui - unsigned clamp (same logic as minsi)
  if (auto minOp = dyn_cast<arith::MinUIOp>(defOp)) {
    Value lhs = minOp.getLhs();
    Value rhs = minOp.getRhs();
    Value lhsTraced = getTracedOrDominatingOperand(lhs);
    ...
    if (lhsTraced && rhsTraced) {
      if (isZeroValue(lhsTraced) || isZeroValue(rhsTraced)) {
        ...
        if (isZeroValue(nonZero))
          return nullptr;
        ARTS_DEBUG("  MinUI includes zero operand - using non-zero operand");
        return nonZero;
      }
...
DbPartitioningPass::partitionAlloc(DbAllocOp allocOp, DbAllocNode *allocNode) {
  ...
  /// If any coarse acquires exist alongside chunked candidates, enable MIXED mode.
  /// This allows chunked partitioning while giving coarse acquires full-range access.
  if (ctx.canChunked) {
    bool hasCoarse =
    ...
    if (hasCoarse && hasChunked) {
      ...
      ARTS_DEBUG("  Mixed mode enabled: chunked allocation with coarse acquires");
  ...
  if (ctx.canChunked) {
    ...
    for (const auto &info : acquireInfos) {
      ...
      Value baseCandidate = extractBaseChunkSizeCandidate(offsetHint, sizeHint);
      if (!baseCandidate)
  ...
  bool allRewriteable = std::all_of(
      acquireInfos.begin(), acquireInfos.end(), [](const auto &info) {
        // Full-range acquires don't need offset/size - they get full allocation
  ...
  if (decision.isChunked()) {
    if (!chunkSizeForPlan) {
      ARTS_DEBUG("  Missing chunk size for chunked decision - keeping original");
      heuristics.recordDecision(
    ...
    if (ValueUtils::isZeroConstant(strippedChunkSize)) {
      ARTS_DEBUG("  Chunk size is zero - keeping original");
      heuristics.recordDecision(
          "Partition-ChunkSizeZero", false,
          "chunk size is zero, keeping original allocation",
          allocOp.getOperation(), {});
      return allocOp;
    }
  }
  ...
  bool hasMixedMode = llvm::any_of(acquireInfos, [](const auto &info) {
    return info.needsFullRange;
  });
  ...
  for (const auto &info : acquireInfos) {
    ...
    if (info.needsFullRange) {
      ...
      // elemOffset/elemSize left empty - will be set to 0/numChunks in rewriter
