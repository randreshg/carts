//===----------------------------------------------------------------------===//
// DbPlacement.cpp - Greedy DbAlloc placement scorer
//===----------------------------------------------------------------------===//
...
llvm::SmallVector<std::string, 8> DbPlacementHeuristics::makeNodeNames(unsigned count) {
...
DbPlacementHeuristics::compute(func::FuncOp func, const llvm::SmallVector<std::string, 8> &nodes) {
  llvm::SmallVector<DbPlacementDecision, 16> decisions;
  if (!dbGraph) return decisions;
  ...
  func.walk([&](EdtOp edt) {
    ...
    std::string n = (edtNodeAttr ? ("N" + std::to_string(edtNodeAttr.getInt())) : std::string("N0"));
  ...
  dbGraph->forEachNode([&](NodeBase *node) {
    if (auto *an = dyn_cast<DbAllocNode>(node)) {
      ...
      DbPlacementDecision d; d.dbAllocOp = allocOp;
      auto &counts = allocToNodeReads[allocOp];
      uint64_t best = 0; std::string bestNode = "N0";
      for (auto &nn : nodes) {
        uint64_t r = counts.lookup(nn);
        d.candidates.push_back({nn, (double)r, r});
        if (r > best) { best = r; bestNode = nn; }
      }
...
void DbPlacementHeuristics::exportToJson(func::FuncOp func,
  ...
  for (auto &d : decisions) {
    if (!first) os << ",\n";
    first = false;
    os << "    {\"alloc\": \"" << d.dbAllocOp << "\", \"chosen\": \"" << d.chosenNode << "\", \"candidates\": [";
    bool fc = true;
