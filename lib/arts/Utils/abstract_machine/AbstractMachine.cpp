...
///===----------------------------------------------------------------------===///
/// AbstractMachine Implementation
///===----------------------------------------------------------------------===///

AbstractMachine::AbstractMachine(const std::string &configFile) {
...
bool AbstractMachine::parseFromFile(const std::string &path) {
...
std::string AbstractMachine::getMachineDescription() const {
...
std::string AbstractMachine::getConfigurationSummary() const {
  std::ostringstream oss;
...
std::string AbstractMachine::trim(const std::string &s) {
  size_t i = 0, j = s.size();
...
bool AbstractMachine::startsWith(const std::string &s, const char *prefix) {
  size_t n = std::strlen(prefix);
  return s.size() >= n && std::equal(prefix, prefix + n, s.begin());
}

std::vector<std::string> AbstractMachine::splitCSV(const std::string &s) {
...
int AbstractMachine::parseInt(const std::string &value, int defaultValue) {
  if (value.empty())
    return defaultValue;
...
bool AbstractMachine::validateConfiguration() {
