...
void IdRegistry::initializeFromModule(ModuleOp module) {
  ...
  for (const auto &entry : locations) {
    locationCache_[entry.loc.key] = nextId_;
    usedIds_.insert(nextId_);
    ARTS_DEBUG("IdRegistry: ID=" << nextId_ << " -> " << entry.loc.key);
    nextId_++;
  }

  initialized_ = true;
  ARTS_DEBUG("IdRegistry: Initialized " << locationCache_.size()
                                        << " mappings, next ID: " << nextId_);
}
...
void IdRegistry::loadFromJsonObject(const llvm::json::Object &root) {
  ...
  auto loadSection = [&](llvm::StringRef sectionName,
    ...
    for (const auto &entry : *section) {
      ...
      locationCache_[locKey] = *idVal;
      usedIds_.insert(*idVal);
      nextId_ = std::max(nextId_, *idVal + 1);
  ...
  initialized_ = true;
  ARTS_DEBUG("IdRegistry: Loaded " << locationCache_.size()
                                   << " IDs from JSON, next ID: " << nextId_);
}
...
void IdRegistry::set(Operation *op, int64_t id) {
  ...
  LocationMetadata loc = LocationMetadata::fromLocation(op->getLoc());
  if (!loc.key.empty())
    locationCache_.try_emplace(loc.key, id);
}
...
int64_t IdRegistry::computeFromLocation(Operation *op) {
  ...
  if (auto attr = op->getAttrOfType<IntegerAttr>(AttrName)) {
    ...
    usedIds_.insert(existing);
    LocationMetadata loc = LocationMetadata::fromLocation(op->getLoc());
    if (!loc.key.empty())
      locationCache_.try_emplace(loc.key, existing);
    return existing;
  }
...
int64_t IdRegistry::assignFromLocation(Operation *op) {
  ...
  // Check cache
  if (auto it = locationCache_.find(loc.key); it != locationCache_.end()) {
...
int64_t IdRegistry::getIdForLocation(llvm::StringRef locationKey) const {
  auto it = locationCache_.find(locationKey);
  return it != locationCache_.end() ? it->second : 0;
}
...
bool IdRegistry::isIdUsed(int64_t id) const { return usedIds_.contains(id); }
