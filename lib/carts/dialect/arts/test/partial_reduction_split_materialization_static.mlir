...
module {
  func.func @partial_reduction_split_materialization_static() {
...
// CHECK-LABEL: func.func @partial_reduction_split_materialization_static
// CHECK: %[[PARTIAL_GUID:[A-Za-z0-9_]+]], %[[PARTIAL_PTR:[A-Za-z0-9_]+]] = arts.db_alloc{{.*}}<block>{{.*}}sizes[{{.*}}, {{.*}}]{{.*}}depPattern = #arts.dep_pattern<reduction>
// CHECK: %[[TREE_GUID:[A-Za-z0-9_]+]], %[[TREE_PTR:[A-Za-z0-9_]+]] = arts.db_alloc{{.*}}<block>{{.*}}sizes[{{.*}}, {{.*}}]{{.*}}depPattern = #arts.dep_pattern<reduction>
// CHECK: scf.for
// CHECK: arts.db_acquire[<out>] (%[[PARTIAL_GUID]] : {{.*}}, %[[PARTIAL_PTR]]
// CHECK: arts.edt <task>
// CHECK-SAME: partialReduction
// CHECK-SAME: partialReductionSplitFactor = 3 : i64
