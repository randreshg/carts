...
module attributes {arts.runtime_total_nodes = 4 : i64, arts.runtime_total_workers = 64 : i64} {
  func.func @internode_planned_task_rejects_coarse_db() {
    ...
    %guid, %ptr = arts.db_alloc[<inout>, <heap>, <write>, <coarse>] route(%route : i32) sizes[%c1] elementType(f64) elementSizes[%c256, %c256] : (memref<?xi64>, memref<?xmemref<?x?xf64>>)
  ...
  func.func @internode_task_without_block_shape_rejects_coarse_db() {
    ...
    %route = arith.constant 0 : i32
    %guid, %ptr = arts.db_alloc[<inout>, <heap>, <write>, <coarse>] route(%route : i32) sizes[%c1] elementType(f64) elementSizes[%c32768] : (memref<?xi64>, memref<?xmemref<?xf64>>)
