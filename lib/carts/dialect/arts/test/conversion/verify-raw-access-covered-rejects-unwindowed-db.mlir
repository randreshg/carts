...
// The standalone verify-raw-access-covered gate is the SDE-to-ARTS boundary's
// fail-closed coverage check: every raw DB-backed access inside an
// sde.su_iterate body must already be covered by a committed SDE
// access-window dependency. Here the `out` DB is windowed, but the `in` DB is
// read without any committed window, so the gate must fail closed rather than
// let SDE leave the dependency unauthored for a later layer to repair.
...
module attributes {arts.runtime_total_nodes = 1 : i64, arts.runtime_total_workers = 4 : i64} {
  func.func @reject_unwindowed_db_access() {
