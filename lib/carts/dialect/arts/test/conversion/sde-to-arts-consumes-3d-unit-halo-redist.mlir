// RUN: %carts-compile %s --pass-pipeline='builtin.module(sde-storage-to-arts-db,sde-accesses-to-arts-deps,finalize-sde-to-arts,verify-arts-objects-only)' 2>&1 | %FileCheck %s --implicit-check-not=sde.redist --implicit-check-not=sde.su_iterate --implicit-check-not=arts.db_access_window --implicit-check-not='full-block halo byte-window' --implicit-check-not=local_only
...
module attributes {arts.runtime_total_nodes = 1 : i64, arts.runtime_total_workers = 4 : i64} {
  func.func @consumes_3d_unit_halo_redist() {
    ...
    sde.redist <halo_like> %P : memref<2x3x4x7x5x3xf32> array_id(0) from owner [0, 1, 2] block [1, 1, 1, 7, 5, 3] to owner [0, 1, 2] block [1, 1, 1, 7, 5, 3] halo [1, 1, 1, 0, 0, 0] cost 1024
    sde.su_iterate (%c1, %c1, %c1) to (%c13, %c14, %c11) step (%c7, %c5, %c3) classification(<stencil>) {
    ^bb0(%ib: index, %jb: index, %kb: index):
      sde.cu_region <parallel> {
        sde.mu_access_window read %P : memref<2x3x4x7x5x3xf32> array_id(0) owner_dims(3) block_lo [0, 0, 0] block_hi [2, 3, 4] valid [7, 5, 3]
        sde.mu_access_window write %Q : memref<2x3x4x7x5x3xf32> array_id(1) owner_dims(3) block_lo [0, 0, 0] block_hi [2, 3, 4] valid [7, 5, 3]
