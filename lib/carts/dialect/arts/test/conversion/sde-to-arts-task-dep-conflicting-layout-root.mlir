...
// A task-dep root with two distinct committed writer layouts is ambiguous.
// ARTS must fail closed instead of selecting one SU-level layout or falling
// through to a coarse DB.

// CHECK: conflicting committed block layouts for task dependency storage root

module attributes {arts.runtime_total_nodes = 2 : i64, arts.runtime_total_workers = 8 : i64} {
  func.func @task_dep_conflicting_layout_root() {
    ...
    sde.su_iterate (%c0) to (%c64) step (%c1) classification(<elementwise>) {
    ^bb0(%i: index):
      sde.array_layout_root write %A : memref<64xf32> array_id(1)
      sde.yield
    } {arrayLayout = [{arrayId = 1 : i64, kind = "block_parallel", ownerDims = [0], blockShape = [8], muBlockCount = 8 : i64, role = "write"}]}
