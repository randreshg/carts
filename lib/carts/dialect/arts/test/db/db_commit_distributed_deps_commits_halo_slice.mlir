...
// A distributed stencil acquire gets its per-slot halo window committed as a
// halo_slice fact, projected from the committed stencil access window.

// CHECK-LABEL: func.func @distributed_stencil_acquire_commits_halo_slice
// CHECK: arts.db_acquire
// CHECK-SAME: halo_slice = #arts.halo_slice<lower = [-1], upper = [1]>

module {
  func.func @distributed_stencil_acquire_commits_halo_slice() {
