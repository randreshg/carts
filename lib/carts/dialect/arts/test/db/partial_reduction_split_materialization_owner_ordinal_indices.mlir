...
module {
  func.func @partial_reduction_split_materialization_owner_ordinal_indices() {
    ...
    %result_guid, %result_ptr = arts.db_alloc[<inout>, <heap>, <write>, <block>]
      route(%route : i32) sizes[%c16] elementType(f32) elementSizes[%c1]
    ...
    %input_guid, %input_ptr = arts.db_alloc[<in>, <heap>, <read>, <block>]
      route(%route : i32) sizes[%c16] elementType(f32) elementSizes[%c1, %c8]
      {planLogicalWorkerSlice = [1], planOwnerDims = [0], planPhysicalBlockShape = [1]}
      : (memref<?xi64>, memref<?xmemref<?x?xf32>>)
    scf.for %owner = %c0 to %c16 step %c4 {
...
// CHECK-LABEL: func.func @partial_reduction_split_materialization_owner_ordinal_indices
// CHECK: %[[PARTIAL_GUID:[A-Za-z0-9_]+]], %[[PARTIAL_PTR:[A-Za-z0-9_]+]] = arts.db_alloc{{.*}}sizes[{{.*}}, {{.*}}]{{.*}}depPattern = #arts.dep_pattern<reduction>
// CHECK: %[[TREE_GUID:[A-Za-z0-9_]+]], %[[TREE_PTR:[A-Za-z0-9_]+]] = arts.db_alloc{{.*}}sizes[{{.*}}, {{.*}}]{{.*}}depPattern = #arts.dep_pattern<reduction>
// CHECK: scf.for %[[OWNER:[A-Za-z0-9_]+]] =
// CHECK: %[[REL:[A-Za-z0-9_]+]] = arith.subi %[[OWNER]],
// CHECK: %[[ORD:[A-Za-z0-9_]+]] = arith.divui %[[REL]],
// CHECK: scf.for %[[TILE:[A-Za-z0-9_]+]] =
// CHECK: arts.db_acquire[<out>] (%[[PARTIAL_GUID]] : {{.*}}, %[[PARTIAL_PTR]] : {{.*}}) partitioning(<block>), indices[%[[ORD]], %[[TILE]]]
// CHECK: arts.db_acquire[<out>] (%[[TREE_GUID]] : {{.*}}, %[[TREE_PTR]] : {{.*}}) partitioning(<block>), indices[%[[ORD]],
// CHECK: arts.db_acquire[<in>] (%[[PARTIAL_GUID]] : {{.*}}, %[[PARTIAL_PTR]] : {{.*}}) partitioning(<block>), indices[%[[ORD]],
// CHECK: arts.db_acquire[<in>] (%[[TREE_GUID]] : {{.*}}, %[[TREE_PTR]] : {{.*}}) partitioning(<block>), indices[%[[ORD]],
