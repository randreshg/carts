...
module attributes {arts.runtime_total_nodes = 4 : i64, arts.runtime_total_workers = 256 : i64} {
  func.func @localizes_rejected_block_db() {
    ...
    %guid, %ptr = arts.db_alloc[<inout>, <heap>, <write>, <block>] route(%route : i32) sizes[%c4] elementType(f64) elementSizes[%c1] {distributed_reject_reason = "unsupported_ptr_users", local_only, planOwnerDims = [0], planPhysicalBlockShape = [1]} : (memref<?xi64>, memref<?xmemref<?xf64>>)
  ...
  func.func @keeps_small_readonly_coarse_db() {
    ...
    %guid, %ptr = arts.db_alloc[<in>, <heap>, <read>, <coarse>] route(%route : i32) sizes[%c1] elementType(f64) elementSizes[%c16] {distributed_reject_reason = "single_block"} : (memref<?xi64>, memref<?xmemref<?xf64>>)
