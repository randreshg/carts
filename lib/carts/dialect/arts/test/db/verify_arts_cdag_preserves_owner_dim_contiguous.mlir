...
// A realized owner_dim_contiguous distributed DB with a committed acquire mode
// passes the CDAG verifier with its owner-map facts preserved.

// CHECK-LABEL: func.func @owner_dim_contiguous_passes_cdag
// CHECK: arts.db_alloc
...
module {
  func.func @owner_dim_contiguous_passes_cdag() {
    ...
    %acq_guid, %acq_ptr = arts.db_acquire[<inout>] (%guid : memref<?xi64>, %ptr : memref<?xmemref<?xf64>>) partitioning(<block>), indices[%c0] {runtime_db_mode = #arts.runtime_db_mode<ew>} -> (memref<?xi64>, memref<?xmemref<?xf64>>)
