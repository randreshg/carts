// RUN: not %carts-compile %s --pipeline post-db-refinement --start-from post-db-refinement --arts-config %inputs_dir/arts_multinode.cfg 2>&1 \
// RUN:   | %FileCheck %s

// A DB cannot enter the distributed lowering path without an explicit owner
// map plan. The canonical pipeline must reject this before
