// RUN: %carts-compile %s --pipeline post-db-refinement --start-from post-db-refinement --arts-config %inputs_dir/arts_multinode.cfg | %FileCheck %s --implicit-check-not=local_only --implicit-check-not=distributed_reject_reason
