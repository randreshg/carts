...
// Passing an acquire to an EDT does not make an extra opaque host use legal.

// CHECK: observes a committed SDE block-layout DB through a non-EDT/non-cleanup use

module {
  func.func @mixed_opaque_host_acquire_committed_layout_rejected() {
    ...
    %acq_guid, %acq_ptr = arts.db_acquire[<in>] (%guid : memref<?xi64>, %ptr : memref<?xmemref<?xf64>>) partitioning(<block>), indices[], offsets[%c0], sizes[%c1] {runtime_db_mode = #arts.runtime_db_mode<ro>} -> (memref<?xi64>, memref<?xmemref<?xf64>>)
    ...
    func.call @opaque(%acq_ptr) : (memref<?xmemref<?xf64>>) -> ()
    arts.db_release(%acq_ptr) : memref<?xmemref<?xf64>>
    return
  }

  func.func private @opaque(memref<?xmemref<?xf64>>)
}
