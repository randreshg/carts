// RUN: %carts-compile %s --pipeline post-db-refinement --start-from post-db-refinement --arts-config %inputs_dir/arts_1t.cfg | %FileCheck %s --implicit-check-not="partitioning(<block>"

// An EDT must not carry both a parent/coarse dependency and a child/block
// dependency for the same root DB.  The post-DB EDT cleanup collapses that
// mixed shape to one conservative coarse dependency.

// CHECK-LABEL: func.func @coarsen_mixed_root_dependencies
// CHECK: %[[REP_GUID:.*]], %[[REP_PTR:.*]] = arts.db_acquire[<inout>] {{.*}} partitioning(<coarse>)
// CHECK: arts.edt <task> <intranode> route({{.*}}) (%[[REP_PTR]]) : memref<?xmemref<?xf64>>

module {
  func.func @coarsen_mixed_root_dependencies() {
