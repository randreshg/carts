...
/// host bridge / small replicated read). This runs after DbOwnerMapRealization
...
static LogicalResult verifyDistributedDbAlloc(DbAllocOp alloc) {
  if (!hasDistributedDbAllocation(alloc.getOperation())) {
    /// owner_scattered is the realized-distributed home; a DB that is not
    /// distributed must not claim it.
    if (alloc.getDbMemoryPlacement() == DbMemoryPlacement::owner_scattered)
      return alloc.emitOpError()
             << "is not distributed but claims owner_scattered placement";
    return success();
  }
  ...
  case DbOwnerMapKind::explicit_rank_table:
    return alloc.emitOpError() << ownerMapKindUnrealizableReason(plan->kind);
  }

  /// A realized distributed DB must carry its scattered home as readable
  /// evidence. A missing or non-scattered placement here is a silent coarse /
  /// node-local fallback for SDE-partitioned storage, which ARTS rejects rather
  /// than lower.
  auto placement = alloc.getDbMemoryPlacement();
  if (!placement)
    return alloc.emitOpError()
           << "is distributed but lacks db_memory_placement evidence";
  if (*placement != DbMemoryPlacement::owner_scattered)
    return alloc.emitOpError()
           << "is distributed but db_memory_placement is not owner_scattered";
