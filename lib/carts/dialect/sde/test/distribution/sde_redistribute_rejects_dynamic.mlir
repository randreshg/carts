...
func.func @dynamic(%T: memref<?x256xf32>, %E: memref<?x256xf32>,
  ...
  sde.cu_region <parallel> {
    ...
    ^bb0(%i: index, %j: index):
      memref.store %cst, %T[%i, %j] : memref<?x256xf32>
      sde.yield
    ...
    ^bb0(%i: index, %j: index):
      scf.for %k = %c0 to %c256 step %c1 {
        %e = memref.load %E[%i, %k] : memref<?x256xf32>
        %t = memref.load %T[%k, %j] : memref<?x256xf32>
        %g = memref.load %G[%i, %j] : memref<?x256xf32>
        %p = arith.mulf %e, %t : f32
        %s = arith.addf %g, %p : f32
        memref.store %s, %G[%i, %j] : memref<?x256xf32>
      }
      sde.yield
