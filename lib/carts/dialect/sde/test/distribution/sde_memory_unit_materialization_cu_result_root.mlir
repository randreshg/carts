// RUN: %carts-compile %s --pass-pipeline='builtin.module(sde-memory-unit-materialization,sde-rank-expand-mu,verify-sde-mu-layout,raise-to-mu-access-window,verify-sde-mu-access-window)' 2>&1 | %FileCheck %s
...
// CHECK-LABEL: func.func @cu_result_root_materializes_visible_mu
...
func.func @cu_result_root_materializes_visible_mu() {
  ...
  } {arrayLayout = [{arrayId = 0 : i64, blockShape = [16], commVolumeBytes = 0 : i64, kind = "block_parallel", muBlockCount = 4 : i64, ownerDims = [0], role = "write"}], physicalOwnerDims = [0], physicalBlockShape = [16]}
  return
}
