...
// owner dims, but the abstract budgeted 2x2 layout-choice grain must not become
// the physical stencil DB/MU grain. SDE commits the realized tiled stencil
// grain for both the input halo DB and the output writer.
...
// CHECK: sde.su_halo %[[IN]] : memref<8x8x32x32xf32> array_id(0) owner [0, 1] block [1, 1, 32, 32] halo [1, 1, 0, 0]
...
func.func @coiterated_stencil_writer_layout() {
  ...
  ^bb0(%i: index, %j: index):
    ...
    sde.cu_region <parallel> {
      ...
      %v = memref.load %src[%im1, %jm1] : memref<256x256xf32>
      memref.store %v, %dst[%i, %j] : memref<256x256xf32>
      sde.yield
    }
    sde.yield
  } {arrayLayout = [{arrayId = 0 : i64, blockShape = [128, 128],
       budgetBlockShape = [17, 17],
       kind = "block_parallel", muBlockCount = 4 : i64, ownerDims = [0, 1],
       role = "read"},
      {arrayId = 1 : i64, blockShape = [128, 128],
       budgetBlockShape = [17, 17],
