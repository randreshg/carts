...
//   * an MU accessed from two distinct CUs has no single canonical window.
//
...
// An MU accessed from two distinct CUs has no single canonical window. Rank
// expansion still converts it (one committed plan), but window raising cannot
// place one window, so it
// skips conservatively (no window in either CU).
// CHECK-LABEL: func.func @fail_closed_multi_cu
func.func @fail_closed_multi_cu() {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c64 = arith.constant 64 : index
  %c128 = arith.constant 128 : index
