...
func.func @ungrounded() {
  ...
  sde.su_distribute <owner_compute> {
    ...
    ^bb0(%i: index):
      ...
      sde.yield
    } {arrayLayout = [{arrayId = 0 : i64, kind = "block_contraction", ownerDims = [0], blockShape = [16, 64], muBlockCount = 8 : i64, role = "read", commVolumeBytes = 0 : i64}], partialReduction, partialReductionDims = [0], partialReductionOwnerDims = [0], physicalBlockShape = [64], physicalOwnerDims = [0]}
  }
  return
}
