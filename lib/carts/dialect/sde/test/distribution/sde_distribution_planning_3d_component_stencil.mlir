...
// ARTS: func.func @main
// ARTS: arts.edt <task> {{.*}}depPattern = #arts.dep_pattern<cross_dim_stencil_3d>{{.*}}planPhysicalBlockShape = [4, 4, 8, 3]{{.*}}stencil_max_offsets = [1, 1, 1, 0]{{.*}}stencil_min_offsets = [-1, -1, -1, 0]{{.*}}stencil_owner_dims = [0, 1, 2, 3]
// ARTS: arts.edt <task> {{.*}}arts.pattern_revision = 1 : i64{{.*}}planPhysicalBlockShape = [4, 4, 8, 3]{{.*}}stencil_owner_dims = [0, 1, 2, 3]
// ARTS-NOT: sde.
