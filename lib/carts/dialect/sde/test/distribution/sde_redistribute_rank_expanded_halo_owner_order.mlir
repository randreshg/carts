...
// CHECK: sde.redist <halo_like> %{{.*}} : memref<4x4x55x72x72x7xf64> array_id(3) from owner [0, 1, 2] block [1, 1, 1, 72, 72, 7] to owner [0, 1, 2] block [1, 1, 1, 72, 72, 7] halo [1, 1, 1, 0, 0, 0] cost 99
...
func.func @expanded_halo_permuted_owner_order(%U: memref<4x4x55x72x72x7xf64>) {
  ...
  } {arrayLayout = [{arrayId = 3 : i64, kind = "block_parallel", ownerDims = [2, 1, 0], blockShape = [144, 288, 7], muBlockCount = 880 : i64, role = "write", commVolumeBytes = 0 : i64}], physicalOwnerDims = [2, 1, 0], physicalBlockShape = [72, 72, 7]}
  ...
  } {arrayLayout = [{arrayId = 3 : i64, kind = "block_parallel", ownerDims = [2, 1, 0], blockShape = [144, 288, 7], muBlockCount = 880 : i64, role = "read", commVolumeBytes = 99 : i64}], layoutsDisagree = [3], physicalOwnerDims = [2, 1, 0], physicalBlockShape = [72, 72, 7], physicalHaloShape = [1, 1, 1]}
  return
}
