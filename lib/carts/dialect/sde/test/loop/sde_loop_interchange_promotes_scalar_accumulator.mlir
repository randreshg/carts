// RUN: %carts-compile %s --O3 --arts-config %arts_config --start-from sde-planning --pipeline codir-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s

// Matrix-like scalar reductions that Polygeist lowers through a rank-0
// memref accumulator should become k-j ordered SDE loops with stride-1 B/C
// accesses in the inner loop.
//
// CHECK-LABEL: // -----// IR Dump After LoopInterchange (loop-interchange) //----- //
// CHECK: func.func @main
// CHECK: sde.su_iterate
// CHECK: scf.for %[[INITJ:[^ ]+]] = %c0 to %c16 step %c1 {
// CHECK: memref.store %{{.*}}, %{{.*}}[%{{.*}}, %[[INITJ]]] : memref<16x16xf32>
// CHECK: scf.for %[[K:[^ ]+]] = %c0 to %c16 step %c1 {
// CHECK: scf.for %[[J:[^ ]+]] = %c0 to %c16 step %c1 {
// CHECK: memref.load %{{.*}}[%{{.*}}, %[[K]]] : memref<16x16xf32>
// CHECK: memref.load %{{.*}}[%[[K]], %[[J]]] : memref<16x16xf32>
// CHECK: memref.load %{{.*}}[%{{.*}}, %[[J]]] : memref<16x16xf32>
// CHECK: arith.addf
// CHECK: memref.store %{{.*}}, %{{.*}}[%{{.*}}, %[[J]]] : memref<16x16xf32>

module attributes {dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<f32, dense<32> : vector<2xi64>>, #dlti.dl_entry<i64, dense<64> : vector<2xi64>>, #dlti.dl_entry<i32, dense<32> : vector<2xi64>>, #dlti.dl_entry<!llvm.ptr, dense<64> : vector<4xi64>>, #dlti.dl_entry<"dlti.endianness", "little">, #dlti.dl_entry<"dlti.stack_alignment", 128 : i64>>, llvm.data_layout = "e-m:e-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128", llvm.target_triple = "aarch64-unknown-linux-gnu"} {
  func.func @main(%A: memref<16x16xf32>, %B: memref<16x16xf32>, %C: memref<16x16xf32>) {
    %c0 = arith.constant 0 : index
    %c16 = arith.constant 16 : index
    %c1 = arith.constant 1 : index
    %zero = arith.constant 0.0 : f32
    omp.parallel {
      omp.wsloop {
        omp.loop_nest (%i) : index = (%c0) to (%c16) step (%c1) {
          %acc = memref.alloca() : memref<f32>
          scf.for %j = %c0 to %c16 step %c1 {
            memref.store %zero, %acc[] : memref<f32>
            scf.for %k = %c0 to %c16 step %c1 {
              %a = memref.load %A[%i, %k] : memref<16x16xf32>
              %b = memref.load %B[%k, %j] : memref<16x16xf32>
              %prod = arith.mulf %a, %b : f32
              %old = memref.load %acc[] : memref<f32>
              %next = arith.addf %old, %prod : f32
              memref.store %next, %acc[] : memref<f32>
            }
            %sum = memref.load %acc[] : memref<f32>
            memref.store %sum, %C[%i, %j] : memref<16x16xf32>
          }
          omp.yield
        }
      }
      omp.terminator
    }
    return
  }
}
