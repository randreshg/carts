...
/// producer and consumer layout disagree into first-class SU movement ops
/// (`sde.su_halo` or `sde.su_reduce_scatter`), or rejects it in SDE.
///==========================================================================///
...
struct SdeRedistributePass
  ...
  void runOnOperation() override {
    ...
    for (const carts::sde::RedistributionEdge &edge : committed.edges) {
      ...
      consumer.emitOpError()
          << "sde-redistribute: movement family "
          << stringifySdeMovementFamily(edge.family)
          << " is not yet realizable; use "
          << carts::sde::suMovementReplacementForFamily(edge.family);
      failed = true;
    }
