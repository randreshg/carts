...
///      the body (no cross-iteration RAW/WAR). Any loop-carried scalar must be a
///      recognized LINEAR INDUCTION COUNTER (`int c=init; ...; c+=K;`), which is
///      a closed-form affine function of the iteration index, not a true
...
static std::optional<ParallelNest> matchParallelNest(scf::ForOp outer,
  ...
  //  - exactly one store to an EXTERNAL array at indices == IVs (the array write)
    ...
    if (isScalarScratch(r)) {
      ...
      if (auto ld = add.getLhs().getDefiningOp<memref::LoadOp>();
        ...
        kc = matchConstInt(add.getRhs());
                 ...
                 ld2 && ValueAnalysis::stripMemrefViewOps(ld2.getMemref()) == r) {
  ...
  // reads. This excludes copies (`u[i][j] = unew[i][j]`), stencils, and any loop
  // that reads another array — those interact with distribution differently
  // (e.g. the double-buffer copy in jacobi-for hits the slice/ownership runtime
  // boundary) and must be left to the normal SDE path. Fail closed.
...
// Compute the per-dim row-major linear stride: stride[d] = prod(extents[d+1..]).
...
static void raiseNest(ParallelNest &nest, OpBuilder &builder) {
  ...
  // Raise ONLY the outermost loop to a 1-D su_iterate (owner dim 0), keeping the
  // inner loops nested as scf.for. This mirrors how a kernel `omp parallel for`
  // is lowered (su_iterate over the outer dim, inner loops stay scf.for), so the
  // written array's owner dim matches the kernel's and LayoutAssignment keeps it
  // block_parallel instead of coarsing. Collapsing to a flat space would instead
  // disagree with the kernel's owner-dim shape and force a coarse fallback.
  ...
  auto suIter = sde::SdeSuIterateOp::create(
      ...
      /*ownerDims=*/nullptr, /*spatialDims=*/nullptr, /*writeFootprint=*/nullptr,
      ...
      /*asyncStrategy=*/nullptr, /*cps_group_id=*/nullptr,
      /*cps_stage_index=*/nullptr, /*cps_stage_count=*/nullptr,
  ...
  // Clone the outer loop body (the inner nest) into the su_iterate body, mapping
  // the outer IV to the su_iterate IV. Inner scf.for loops (and their IVs) are
  // cloned verbatim.
  ...
  // Replace counter loads with init + step * (row-major linear index), and erase
  // counter increment stores. The IVs are all in scope at the innermost body.
  for (const Counter &c : nest.counters) {
    ...
    for (memref::LoadOp ld : ldToReplace) {
      ...
      Value scaled =
          c.step == 1
              ? lin
              : arith::MulIOp::create(b, loc, lin,
                                      createConstantIndex(b, loc, c.step));
      Value sum = c.init == 0
                      ? scaled
                      : arith::AddIOp::create(
                            b, loc, scaled, createConstantIndex(b, loc, c.init));
      Value val = sum;
