...
getCodirTileOwnerBlockSizes(codir::CodeletOp codelet, unsigned depIndex,
  ...
  for (auto [slot, ownerDim] : llvm::enumerate(*ownerDims)) {
    ...
    } else if (std::optional<SmallVector<int64_t, 4>> spatialDims =
                   readI64ArrayAttr(codelet.getSpatialDimsAttr());
               spatialDims && blockShape->size() == spatialDims->size()) {
      for (auto [spatialSlot, spatialDim] : llvm::enumerate(*spatialDims)) {
        if (spatialDim < 0 || static_cast<unsigned>(spatialDim) != ownerDim)
          continue;
        blockSize = (*blockShape)[spatialSlot];
        break;
      }
