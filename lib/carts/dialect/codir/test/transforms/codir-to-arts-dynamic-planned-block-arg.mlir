...
// RUN:   | %FileCheck %s --implicit-check-not="arts.edt <task> <intranode>"

// Dynamic function arguments still have concrete runtime extents available via
// memref.dim. CODIR-to-ARTS must preserve those extents when materializing an
// SDE/CODIR-authored physical block layout; otherwise planned dynamic inputs
// fall back to coarse storage or fail before they can be routed across nodes.
