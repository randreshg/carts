...
// An owner-derived index one past the grouped logical window would select a DB
// block outside the acquired window. Reject it instead of materializing an
// unchecked db_ref.

module attributes {arts.runtime_total_nodes = 4 : i64, arts.runtime_total_workers = 256 : i64} {
  func.func @reject_out_of_window_grouped_index() {
      ...
      scf.for %j = %c0 to %c16 step %c8 {
        scf.for %k = %c0 to %c16 step %c8 {
          ...
          ^bb0(%arg0: memref<16x16x16xf32>, %iBase: index, %jBase: index, %kBase: index):
            ...
            %tooFar = arith.addi %iBase, %inner_c8 : index
            memref.store %value, %arg0[%tooFar, %jBase, %kBase] : memref<16x16x16xf32>
            codir.yield
          }
        }
      }
