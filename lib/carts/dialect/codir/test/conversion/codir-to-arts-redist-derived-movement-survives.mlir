...
// RUN: %carts-compile %s --pass-pipeline='builtin.module(convert-sde-to-codir,verify-codir,codir-codelet-dce,reduction-dep-mapping,dep-storage-assignment,verify-codir,convert-sde-boundary-to-arts,convert-codir-to-arts,realize-edt-distribution-plan,verify-arts-objects-only)' \
...
// the reader's dep_collectives carrier and survives codelet-opt, reduction
// planning, and storage planning; CODIR-to-ARTS then consumes that committed
// movement into ARTS objects, leaving no sde.redist carrier behind. The reader
// layout here equals the writer home, so the consumed read is realized as a
// coarse acquire (the documented layout-assignment limitation that discards the
// reader's required target); this pins that realized shape rather than implying
// a block redistribution.

// CODIR: dep_collectives = [#codir.collective<none>, #codir.collective<reduce_scatter>]

// ARTS-LABEL: func.func @consume_redist
// ARTS: arts.edt <task>
