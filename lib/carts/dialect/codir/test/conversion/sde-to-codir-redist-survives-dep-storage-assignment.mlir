// RUN: %carts-compile %s --pass-pipeline='builtin.module(convert-sde-to-codir,codir-codelet-dce,reduction-dep-mapping,dep-storage-assignment,verify-codir)' 2>&1 | %FileCheck %s

// The movement family CODIR consumes from a committed sde.redist must survive
// the rest of the CODIR planning pipeline: dep-storage-assignment preserves the
// committed reduce_scatter on the consumer edge instead of reclassifying it
// (chooseCollective would otherwise compute none for a read dependency).
