...
// RUN: %carts-compile %s --start-from codir-to-arts --pipeline post-db-refinement --arts-config %inputs_dir/arts_multinode_4x64.cfg \
...
// CONVERT: arts.edt <task>{{.*}}planOwnerDims = [2, 1, 0]
...
// DIST-SAME: offsets[{{%[^,]+}}, {{%[^,]+}}, {{%[^]]+}}]
// DIST-SAME: sizes[{{%[^,]+}}, {{%[^,]+}}, {{%[^]]+}}]
// DIST: arts.edt <task>{{.*}}planOwnerDims = [2, 1, 0]
