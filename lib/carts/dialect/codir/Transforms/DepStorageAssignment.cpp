///==========================================================================///
/// File: DepStorageAssignment.cpp
///
/// CODIR-owned dependency storage assignment before ARTS materialization.
///==========================================================================///
...
#define GEN_PASS_DEF_DEPSTORAGEASSIGNMENT
...
/// full array, but the halo-exchange fact provides stronger ordering
...
static bool hasFinalizedStorageFacts(codir::CodeletOp codelet) {
  if (!codelet)
    return false;
  unsigned depCount = codelet.getDeps().size();
...
struct DepStorageAssignmentPass
    : public codir::impl::DepStorageAssignmentBase<DepStorageAssignmentPass> {
  void runOnOperation() override {
    ...
    getOperation().walk([&](codir::CodeletOp codelet) {
      ...
      bool finalized = hasFinalizedStorageFacts(codelet);
      if (finalized)
    ...
    getOperation().walk([&](codir::CodeletOp codelet) {
      ...
      if (depCount == 0)
        return;
      // A non-none movement family already committed on a dependency (from
      ...
      for (unsigned index = 0; index < depCount; ++index) {
        codir::CodirCollectiveKindAttr committed =
            committedCollectives && index < committedCollectives.size()
                ? dyn_cast<codir::CodirCollectiveKindAttr>(
...
std::unique_ptr<Pass> mlir::carts::codir::createDepStorageAssignmentPass() {
  return std::make_unique<DepStorageAssignmentPass>();
}
