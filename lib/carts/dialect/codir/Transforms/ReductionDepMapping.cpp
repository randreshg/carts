///==========================================================================///
/// File: ReductionDepMapping.cpp
///
/// CODIR-owned partial-reduction dependency/result mapping.
///==========================================================================///
...
#define GEN_PASS_DEF_REDUCTIONDEPMAPPING
...
static std::optional<int64_t> getWorkerCapacity(ModuleOp module) {
...
static LogicalResult setOrVerifyReductionPlanAttr(Operation *op,
  ...
  if (desired) {
    if (existing && existing != desired)
      return op->emitError()
             << "existing " << factName
             << " does not match recomputed CODIR reduction fact";
    if (!existing)
  ...
  if (existing)
    return op->emitError()
           << "existing " << factName
           << " is set but recomputed CODIR reduction fact has no value";
  return success();
}
...
struct ReductionDepMappingPass
    : public codir::impl::ReductionDepMappingBase<ReductionDepMappingPass> {
  void runOnOperation() override {
    std::optional<int64_t> targetWorkerCount =
        getWorkerCapacity(getOperation());
...
std::unique_ptr<Pass> mlir::carts::codir::createReductionDepMappingPass() {
  return std::make_unique<ReductionDepMappingPass>();
}
