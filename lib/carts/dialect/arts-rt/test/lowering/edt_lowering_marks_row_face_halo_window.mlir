...
// ARTS-RT mechanically lowers an already-committed contiguous row-face element
// window to one nonzero halo byte window.

// CHECK-LABEL: func.func @edt_lowering_marks_row_face_halo_window
...
module attributes {
  ...
  func.func @edt_lowering_marks_row_face_halo_window() {
    ...
    %acq_guid, %acq_ptr = arts.db_acquire[<in>] (%guid : memref<?x?xi64>, %ptr : memref<?x?xmemref<?x?x?x?xf64>>) partitioning(<block>), indices[], offsets[%c0, %c0], sizes[%c1, %c1] element_offsets[%c0, %c0, %c31, %c0] element_sizes[%c1, %c1, %c1, %c32] {haloViewDependency, runtime_db_mode = #arts.runtime_db_mode<ro>, stencil_max_offsets = [0, 0], stencil_min_offsets = [-1, 0], stencil_owner_dims = [0, 1], stencil_spatial_dims = [0, 1], stencil_supported_block_halo} -> (memref<?x?xi64>, memref<?x?xmemref<?x?x?x?xf64>>)

    arts.edt <task> <internode> route(%route) (%acq_ptr) : memref<?x?xmemref<?x?x?x?xf64>> {
    ^bb0(%dep: memref<?x?xmemref<?x?x?x?xf64>>):
      %payload = arts.db_ref %dep[%c0, %c0] : memref<?x?xmemref<?x?x?x?xf64>> -> memref<?x?x?x?xf64>
      %value = memref.load %payload[%c0, %c0, %c0, %c0] : memref<?x?x?x?xf64>
