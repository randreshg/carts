...
// CHECK: cannot recover DB acquire provenance for dependency GUID; refusing single-element DB shape fallback
...
module attributes {
  ...
  func.func @record_dep_raw_guid_storage_fails(%edt: i64, %db: i64) {
    %c0 = arith.constant 0 : index
    %storage = memref.alloca() : memref<1xi64>
    memref.store %db, %storage[%c0] : memref<1xi64>
    arts_rt.rec_dep %edt(%storage : memref<1xi64>) {acquire_modes = array<i32: 1>}
    return
  }
}
