...
// ARTS commits the per-acquire halo-view dependency and explicit element
// window. ARTS-RT pre-lowering consumes that slot-level fact mechanically
// before runtime-call lowering chooses the halo API.
...
module attributes {
  ...
  func.func @edt_lowering_marks_per_block_halo_slice() {
    ...
    %guid, %ptr = arts.db_alloc[<inout>, <heap>, <write>, <stencil>] route(%route : i32) sizes[%c4] elementType(f64) elementSizes[%c16] {distributed} : (memref<?xi64>, memref<?xmemref<?xf64>>)
