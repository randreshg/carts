...
// ARTS commits the per-block halo exchange EDT and explicit element window.
// ARTS-RT pre-lowering consumes those facts mechanically by tagging the
// dependency as a halo view before the runtime-call lowering chooses the halo
// API.

// CHECK-LABEL: func.func @edt_lowering_marks_per_block_halo_slice
...
module attributes {
  ...
  func.func @edt_lowering_marks_per_block_halo_slice() {
    ...
    %acq_guid, %acq_ptr = arts.db_acquire[<in>] (%guid : memref<?xi64>, %ptr : memref<?xmemref<?xf64>>) partitioning(<stencil>), indices[%c0], offsets[%c0], sizes[%c1] element_offsets[%c0] element_sizes[%c1] {halo_slice = #arts.halo_slice<lower = [-1], upper = [0]>, runtime_db_mode = #arts.runtime_db_mode<ro>} -> (memref<?xi64>, memref<?xmemref<?xf64>>)

    arts.edt <task> <internode> route(%route) (%acq_ptr) : memref<?xmemref<?xf64>> attributes {perBlockHaloExchange} {
