...
module attributes {
  ...
  func.func @record_dep_halo_view_zero_size_fails(%edt: i64, %db: i64) {
    %c0 = arith.constant 0 : index
    %storage = memref.alloca() : memref<1xi64>
    memref.store %db, %storage[%c0] : memref<1xi64>
    arts_rt.rec_dep %edt(%storage : memref<1xi64>) byte_offsets(%c0) byte_sizes(%c0) {acquire_modes = array<i32: 1>, dep_flags = array<i32: 4>}
    return
  }
}
