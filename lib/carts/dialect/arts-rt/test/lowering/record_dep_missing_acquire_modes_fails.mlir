...
// CHECK: 'arts_rt.rec_dep' op requires acquire_modes for every datablock; ARTS-RT must not infer DB dependency modes
...
module attributes {
  ...
  func.func @record_dep_missing_acquire_modes_fails(%edt: i64) {
    ...
    arts_rt.rec_dep %edt(%acq_guid : memref<?xi64>)
    return
  }
}
