...
// A committed halo-view dependency whose explicit element window equals the
// full local DB payload still lowers as a nonzero halo byte window. ARTS-RT
// must not normalize that committed full-local halo edge back to a whole-DB
// dependency.

// CHECK-LABEL: func.func @edt_lowering_marks_full_local_halo_window
...
module attributes {
  ...
  func.func @edt_lowering_marks_full_local_halo_window() {
