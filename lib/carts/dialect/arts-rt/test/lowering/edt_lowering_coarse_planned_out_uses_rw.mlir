...
// ARTS-RT maps the explicit ARTS-level runtime DB mode verdict to DB_MODE_RW.
// The same planned coarse output shape without that verdict must stay ordered
// EW in this pass.
...
// CHECK-LABEL: func.func @coarse_planned_out_without_verdict_uses_ew
// CHECK: arts_rt.rec_dep
...
module attributes {
  ...
  func.func @coarse_planned_out_uses_unordered_rw() {
    ...
    %acq_guid, %acq_ptr = arts.db_acquire[<out>] (%guid : memref<?xi64>, %ptr : memref<?xmemref<?xf64>>) partitioning(<coarse>), indices[], offsets[%c0], sizes[%c1] {runtime_db_mode = #arts.runtime_db_mode<rw>} -> (memref<?xi64>, memref<?xmemref<?xf64>>)
  ...
  func.func @coarse_planned_out_without_verdict_uses_ew() {
    %route = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c4 = arith.constant 4 : index
