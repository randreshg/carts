...
sdeMovementFamilyToCollective(sde::SdeMovementFamily family) {
  switch (family) {
  case sde::SdeMovementFamily::broadcast_like:
    return codir::CodirCollectiveKind::broadcast;
  case sde::SdeMovementFamily::all_gather_like:
    return codir::CodirCollectiveKind::all_gather;
  case sde::SdeMovementFamily::all_to_all_like:
    return codir::CodirCollectiveKind::all_to_all;
  case sde::SdeMovementFamily::reduce_scatter_like:
    return codir::CodirCollectiveKind::reduce_scatter;
  case sde::SdeMovementFamily::allreduce_like:
    return codir::CodirCollectiveKind::allreduce;
  case sde::SdeMovementFamily::halo_like:
    return codir::CodirCollectiveKind::halo;
  case sde::SdeMovementFamily::phase_redist:
    return std::nullopt;
  }
  return std::nullopt;
}
...
/// array.
static inline SmallVector<Attribute>
completeDepAttrs(ArrayAttr existing, unsigned count, Attribute fill) {
  SmallVector<Attribute> out;
  out.reserve(count);
  for (unsigned i = 0; i < count; ++i)
    out.push_back(existing && i < existing.size() ? existing[i] : fill);
  return out;
}
...
/// array dependencies but committed no `sde.redist` movement for it. CODIR
/// refuses to classify or invent the movement.
static inline LogicalResult
rejectUncommittedMovement(codir::CodeletOp codelet,
                          ArrayRef<sde::SdeRedistOp> redists) {
  ArrayAttr disagree = codelet.getLayoutsDisagreeAttr();
  if (!disagree || disagree.empty())
    return success();
  ArrayAttr arrayIds = codelet.getDepArrayIdsAttr();
  if (!arrayIds)
    return success();
  for (Attribute idAttr : disagree) {
    auto disId = dyn_cast<IntegerAttr>(idAttr);
    if (!disId)
      continue;
    bool sawDep = false;
    bool covered = false;
    for (auto [i, a] : llvm::enumerate(arrayIds)) {
      auto depId = dyn_cast<IntegerAttr>(a);
    ...
    if (sawDep && !covered)
      return codelet.emitOpError()
             << "consumes committed SDE structure but array " << disId.getInt()
             << " has a recorded layout disagreement with no sde.redist "
                "movement; CODIR will not classify or invent the movement";
  }
  return success();
}
...
static inline LogicalResult consumeCommittedSdeStructure(Operation *root) {
  ...
  root->walk([&](Operation *op) {
    if (auto window = dyn_cast<sde::SdeMuAccessWindowOp>(op))
      windows.push_back(window);
    else if (auto redist = dyn_cast<sde::SdeRedistOp>(op))
      redists.push_back(redist);
  });
  if (windows.empty() && redists.empty())
    return success();

  codir::CodeletDataflowGraph graph(root);

  // Access windows: an owner-local compute-block view with the committed owner
  // grid dims, replacing the coarse host-whole default from conversion.
  for (sde::SdeMuAccessWindowOp window : windows) {
    Value muRoot = ValueAnalysis::stripMemrefViewOps(window.getMu());
    const auto *deps = graph.depsForRoot(muRoot);
    if (!deps)
      continue;
    bool windowWrites = window.getMode() == sde::SdeAccessMode::write;
    auto ownerDimCount = static_cast<unsigned>(window.getOwnerDimCount());
  ...
  // Redistributions: represent the committed movement family on each
  // producer -> consumer graph edge whose array root carries an sde.redist.
  for (const codir::CodeletDataflowGraph::Edge &edge : graph.edges()) {
    codir::CodeletOp consumer = edge.consumer.codelet;
    for (sde::SdeRedistOp redist : redists) {
      if (ValueAnalysis::stripMemrefViewOps(redist.getMu()) != edge.root)
        continue;
      std::optional<codir::CodirCollectiveKind> movement =
...
static inline bool requiresSuCompletionBarrier(sde::SdeSuIterateOp source) {
  return !source.getNowaitAttr();
}

static inline LogicalResult
convertSuOwnerTileNdToCodir(sde::SdeSuIterateOp source,
...
convertSuIterateToCodir(sde::SdeSuIterateOp source,
  ...
  if (getSuOwnerTileDispatchPhysicalDims(source))
    return convertSuOwnerTileNdToCodir(source, depArrayIdPlan);
