...
/// DbAccessOp represents operations that can be used as datablock access operations
/// This includes both SubViewOp (original case) and CastOp (after canonicalization)
struct DbAccessOp {
  mlir::Operation *op;
  ...
  mlir::Value getResult() const { return op ? op->getResult(0) : mlir::Value(); }
  ...
  bool isSubView() const { return op && mlir::isa<mlir::memref::SubViewOp>(op); }
  ...
  mlir::memref::SubViewOp getAsSubView() const {
    return isSubView() ? mlir::cast<mlir::memref::SubViewOp>(op) : mlir::memref::SubViewOp();
  }
  
  /// Get as CastOp (returns null if not a CastOp)
  mlir::memref::CastOp getAsCast() const {
    return isCast() ? mlir::cast<mlir::memref::CastOp>(op) : mlir::memref::CastOp();
  }
};
...
inline DbAccessOp getDbAccessOp(mlir::Value value) {
  ...
  llvm_unreachable("Operation must be either SubViewOp or CastOp for DbAccessOp");
}
...
namespace llvm {
template <>
struct DenseMapInfo<DbAccessOp> {
  static DbAccessOp getEmptyKey() {
    auto ptr = llvm::DenseMapInfo<void *>::getEmptyKey();
    return DbAccessOp(static_cast<mlir::Operation *>(ptr));
  }
  
  static DbAccessOp getTombstoneKey() {
    auto ptr = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return DbAccessOp(static_cast<mlir::Operation *>(ptr));
  }
  
  static unsigned getHashValue(const DbAccessOp &val) {
    return llvm::DenseMapInfo<mlir::Operation *>::getHashValue(val.getOperation());
  }
  
  static bool isEqual(const DbAccessOp &lhs, const DbAccessOp &rhs) {
