...
/// clang++ -fopenmp -std=c++17 simple.cpp  -Xclang -plugin  -Xclang omp-plugin -fplugin=/home/randres/projects/carts/.install/carts/lib/libOpenMPPlugin.so -mllvm -debug-only=omp-plugin -S
...
int main() {
  ...
  outlined_region_2(shared_number, random_number);
...
static void __attribute__((annotate("task"))) outlined_region_1(int shared_number) {
...
static void __attribute__((annotate("parallel"))) outlined_region_2(int shared_number, int random_number) {
#pragma omp parallel
  {
      ...
      /// EDT 3
      /// 0: shared_number 
      outlined_region_1(shared_number);
