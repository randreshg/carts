// RUN: not %carts-compile %s --O3 --arts-config %S/../../examples/arts.cfg --start-from=concurrency --pipeline=edt-distribution 2>&1 | %FileCheck %s

// Verify that VerifyForLowered rejects residual arts.for operations when
// loop lowering did not consume them.
//
// CHECK: arts.for survived past edt-distribution stage
