// RUN: %carts-compile %s --O3 --arts-config %S/../inputs/arts_multinode.cfg --pipeline openmp-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s --check-prefix=SDE
// RUN: %carts-compile %s --O3 --arts-config %S/../inputs/arts_multinode.cfg --pipeline openmp-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s --check-prefix=ARTS
...
// SDE-LABEL: // -----// IR Dump After SdeDistributionPlanning (sde-distribution-planning) //----- //
// SDE: func.func @main
...
// ARTS-SAME: {{.*}}depPattern = #arts.dep_pattern<stencil_tiling_nd>{{.*}}distribution_kind = #arts.distribution_kind<block>{{.*}}distribution_pattern = #arts.distribution_pattern<stencil>{{.*}}distribution_version = 1 : i32
