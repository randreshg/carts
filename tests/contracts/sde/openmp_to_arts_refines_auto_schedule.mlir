...
// Verify that ConvertOpenMPToSde preserves an abstract OpenMP auto schedule
// on SDE, then SdeScheduleRefinement chooses a concrete SDE schedule before
// later chunk or lowering logic runs.

// CHECK-LABEL: // -----// IR Dump After ConvertOpenMPToSde (convert-openmp-to-sde) //----- //
// CHECK: func.func @main
// CHECK: arts_sde.cu_region <parallel> {
// CHECK: arts_sde.su_iterate(%c0) to(%c128) step(%c1) schedule(<auto>)

// CHECK-LABEL: // -----// IR Dump After SdeScheduleRefinement (sde-schedule-refinement) //----- //
// CHECK: func.func @main
// CHECK: arts_sde.cu_region <parallel> scope(<local>) {
// CHECK: arts_sde.su_iterate(%c0) to(%c128) step(%c1) schedule(<static>)
// CHECK-NOT: arts.for
// CHECK: // -----// IR Dump After SdeChunkOptimization (sde-chunk-optimization) //----- //
