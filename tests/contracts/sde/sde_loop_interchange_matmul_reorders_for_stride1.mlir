// RUN: %carts-compile %s --O3 --arts-config %S/../../examples/arts.cfg --pipeline openmp-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s --check-prefix=BEFORE
// RUN: %carts-compile %s --O3 --arts-config %S/../../examples/arts.cfg --pipeline openmp-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s --check-prefix=AFTER

// Verify that SdeLoopInterchange swaps j-k loops to k-j order for stride-1
...
// AFTER-LABEL: // -----// IR Dump After SdeLoopInterchange (sde-loop-interchange) //----- //
