...
// symbolic-trip auto schedule when the same concrete schedule wins across the
// pass's SDE-level probe trip counts. The refinement must stay attribute-only:
// no chunk operand is introduced before or after SdeChunkOptimization.
