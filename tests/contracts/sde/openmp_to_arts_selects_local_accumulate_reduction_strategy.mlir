// RUN: %carts-compile %s --O3 --arts-config %S/../inputs/arts_2t.cfg --pipeline openmp-to-arts --mlir-print-ir-after-all 2>&1 | %FileCheck %s

// Verify that SdeReductionStrategy picks the local-accumulate path when the
// preserved reduction loop already contains nested sequential work.
