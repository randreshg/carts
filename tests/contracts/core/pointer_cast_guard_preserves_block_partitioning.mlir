...
// not require a contiguous whole-view layout. Keep the main `x` tensor
// coarse-allocated with block-partitioned acquires and preserve the null-check
// pointer cast instead of forcing the whole allocation back to coarse.

// CHECK: %[[X_GUID:[^,]+]], %[[X_PTR:[^ ]+]] = arts.db_alloc[<inout>, <heap>, <write>, <coarse>
