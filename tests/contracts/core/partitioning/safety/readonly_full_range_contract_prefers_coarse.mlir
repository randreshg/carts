...
// CHECK: %[[UGUID:.*]], %[[UPTR:.*]] = arts.db_alloc[<in>, <heap>, <read>, <coarse>, <stencil>] route(%c-1_i32 : i32) sizes[%c1]
// CHECK: %[[UACQ_GUID:.*]], %[[UACQ_PTR:.*]] = arts.db_acquire[<in>] (%[[UGUID]] : memref<?xi64>, %[[UPTR]] : memref<?xmemref<?x?x?x?xf32>>) partitioning(<block>, offsets[%{{.+}}], sizes[%{{.+}}]), offsets[%{{.+}}], sizes[%{{.+}}] {{.*}}distribution_pattern = #arts.distribution_pattern<uniform>{{.*}}stencil_owner_dims = [3]
