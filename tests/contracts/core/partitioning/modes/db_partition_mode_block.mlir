...
// coarse allocations with block-partitioned acquires for both
// the read input and the write output.
// CHECK-LABEL: func.func @main
// CHECK: %[[IN_GUID:.*]], %[[IN_PTR:.*]] = arts.db_alloc[{{.*}}<coarse>, <uniform>{{.*}}]
// CHECK: %[[OUT_GUID:.*]], %[[OUT_PTR:.*]] = arts.db_alloc[{{.*}}<coarse>, <uniform>{{.*}}]
...
// CHECK-SAME: ]), offsets[
// CHECK: arts.db_acquire[<inout>] (%[[OUT_GUID]] : memref<?xi64>, %[[OUT_PTR]] : memref<?xmemref<?x?xf32>>) partitioning(<block>, offsets[
// CHECK-SAME: ]), offsets[
