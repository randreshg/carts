// RUN: %carts-run %s --O3 --arts-config %S/../../docker/arts-docker-2node.cfg --distributed-db-ownership --stop-at concurrency-opt --debug-only=distributed_db_ownership 2>&1 | %FileCheck %s
