...
// CHECK: %[[FG:.*]], %[[FP:.*]] = arts.db_alloc[<inout>, <heap>, <write>, <block>, <indexed>]
// CHECK: sizes[%[[FBLOCKS:.*]]] elementType(f64)
// CHECK: arts.db_acquire[<inout>] (%[[FG]] : memref<?xi64>,
// CHECK: partitioning(<block>, offsets[%{{.*}}], sizes[%{{.*}}]),
// CHECK: offsets[%{{.*}}], sizes[%{{.*}}] {{.*}}-> (memref<?xi64>, memref<?xmemref<?x?xf64>>)
