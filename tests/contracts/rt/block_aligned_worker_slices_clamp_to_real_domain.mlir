...
// CHECK: %guid, %ptr = arts.db_alloc[<in>, <heap>, <read>, <coarse>] {{.*}} elementSizes[%c3, %c320, %c320, %c576]
...
// CHECK: arts.db_acquire[<in>] {{.*}} partitioning(<block>, offsets[%{{.+}}], sizes[%{{.+}}]), offsets[%{{.+}}], sizes[%{{.+}}] {{.*}}distribution_pattern = #arts.distribution_pattern<uniform>{{.*}}stencil_owner_dims = [2]
// CHECK: arts.lowering_contract({{.*}}) pattern(<{{.*}}distributionKind = <block>, distributionPattern = <uniform>{{.*}}) block_shape[%c320] contract(<ownerDims = [2], postDbRefined = true, criticalPathDistance = 0 : i64>)
