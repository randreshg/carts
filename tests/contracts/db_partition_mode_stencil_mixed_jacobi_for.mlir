// RUN: %carts-compile %s --O3 --arts-config %S/../examples/arts.cfg --stop-at concurrency-opt | %FileCheck %s --check-prefix=LOCAL
// RUN: %carts-compile %s --O3 --arts-config %S/../../docker/arts-docker-2node.cfg --stop-at concurrency-opt | %FileCheck %s --check-prefix=MULTI

// LOCAL: %[[SGUID:.*]], %[[SPTR:.*]] = arts.db_alloc[<inout>, <heap>, <write>, <stencil>, <stencil>]
// LOCAL: arts.db_acquire[<inout>] (%[[SGUID]] : memref<?xi64>, %[[SPTR]] : memref<?xmemref<?x?xf64>>) partitioning(<block>)
// LOCAL-NOT: arts.db_acquire[<inout>] (%[[SGUID]] : memref<?xi64>, %[[SPTR]] : memref<?xmemref<?x?xf64>>) partitioning(<coarse>)
// LOCAL: arts.db_acquire[<in>] (%[[SGUID]] : memref<?xi64>, %[[SPTR]] : memref<?xmemref<?x?xf64>>) partitioning(<block>)
// LOCAL: stencil_center_offset = 1 : i64

// MULTI: %[[SGUID:.*]], %[[SPTR:.*]] = arts.db_alloc[<inout>, <heap>, <write>, <stencil>, <stencil>]
// MULTI: arts.db_acquire[<inout>] (%[[SGUID]] : memref<?xi64>, %[[SPTR]] : memref<?xmemref<?x?xf64>>) partitioning(<block>)
// MULTI: arts.db_acquire[<in>] (%[[SGUID]] : memref<?xi64>, %[[SPTR]] : memref<?xmemref<?x?xf64>>) partitioning(<block>)
// MULTI: stencil_center_offset = 1 : i64
