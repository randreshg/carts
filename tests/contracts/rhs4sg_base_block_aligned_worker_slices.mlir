...
// the worker-local blocked acquire contracts even though the backing DB
// allocation now remains coarse until outlining.
...
// CHECK: arts.db_acquire[<in>] {{.*}} partitioning(<block>, offsets[%{{.+}}], sizes[%{{.+}}]), offsets[%{{.+}}], sizes[%{{.+}}] {{.*}}distribution_pattern = #arts.distribution_pattern<uniform>{{.*}}stencil_owner_dims = [3]
