...
// CHECK: arts.db_acquire[<inout>] {{.*}} partitioning(<block>), offsets[%{{.+}}], sizes[%{{.+}}] {arts.pattern_revision = 1 : i64, depPattern = #arts.dep_pattern<wavefront_2d>, distribution_kind = #arts.distribution_kind<block>, distribution_pattern = #arts.distribution_pattern<stencil>, distribution_version = 1 : i32, stencil_block_shape = [253, 600], stencil_owner_dims = [0], stencil_spatial_dims = [0, 1], stencil_supported_block_halo, stencil_write_footprint = [0]}
// CHECK-NEXT: arts.lowering_contract(%{{.+}} : memref<?x!llvm.ptr>) pattern(<depPattern = <wavefront_2d>, distributionKind = <block>, distributionPattern = <stencil>, distributionVersion = 1 : i64, revision = 1 : i64>) block_shape[%c253] min_offsets[%c-1] max_offsets[%c1] write_footprint[%c0] contract(<ownerDims = [0], centerOffset = 1 : i64, spatialDims = [0, 1], supportedBlockHalo = true{{.*}})
