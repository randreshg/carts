// RUN: %carts-run %s --O3 --arts-config %S/../../docker/arts-docker-2node.cfg --serial-edtify --stop-at loop-reordering --debug-only=serial_edtify 2>&1 | %FileCheck %s --check-prefix=EDTIFY
// RUN: %carts-run %s --O3 --arts-config %S/../../docker/arts-docker-2node.cfg --serial-edtify --distributed-db-ownership --stop-at concurrency-opt --debug-only=serial_edtify,distributed_db_ownership 2>&1 | %FileCheck %s --check-prefix=DIST
// RUN: %carts-run %s --O3 --arts-config %S/../../docker/arts-docker-2node.cfg --distributed-db-ownership --stop-at concurrency-opt --debug-only=serial_edtify,distributed_db_ownership 2>&1 | %FileCheck %s --check-prefix=AUTO
...
// DIST: DistributedDbOwnership marked 1 / 1 DbAlloc operations
// DIST: arts.db_alloc
// DIST-SAME: distributed

// AUTO: SerialEdtify outlined 1 / 1 eligible serial loop(s)
// AUTO: DistributedDbOwnership marked 1 / 1 DbAlloc operations
// AUTO: arts.db_alloc
// AUTO-SAME: distributed
