// RUN: %carts-compile %s --arts-config %S/../examples/arts.cfg --arts-epoch-finish-continuation --start-from=epochs --stop-at=pre-lowering | %FileCheck %s --check-prefix=CONT
// RUN: %carts-compile %s --arts-config %S/../examples/arts.cfg --start-from=epochs --stop-at=pre-lowering | %FileCheck %s --check-prefix=WAIT
