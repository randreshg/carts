...
// CHECK: %[[CENTER:.+]] = arith.cmpi eq, %arg5, %{{.+}} : index
// CHECK-NEXT: %[[MODE:.+]] = arith.select %[[CENTER]], %c2_i32, %c1_i32 : i32
// CHECK: %[[BYTE_SIZE:.+]] = arith.select %[[CENTER]], %c0_i64, %{{.+}} : i64
// CHECK: %[[USE_WHOLE:.+]] = arith.cmpi eq, %[[BYTE_SIZE]], %c0_i64 : i64
// CHECK: scf.if %[[USE_WHOLE]] {
// CHECK: func.call @arts_add_dependence_ex({{.*}}%[[MODE]], %c2_i32) : (i64, i64, i32, i32, i32) -> ()
// CHECK: } else {
// CHECK: %[[BYTE_OFF:.+]] = arith.select %[[CENTER]], %c0_i64, %{{.+}} : i64
// CHECK: func.call @arts_add_dependence_at_ex({{.*}}%[[MODE]], %[[BYTE_OFF]], %[[BYTE_SIZE]], %c2_i32) : (i64, i64, i32, i32, i64, i64, i32) -> ()
// CHECK: }
