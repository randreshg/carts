# jacobi example analysis

Walk through these steps and fix any problem that you find in the way

1. **Navigate to the jacobi example directory:**

   ```bash
   cd ~/Documents/carts/external/carts-benchmarks/kastors-jacobi/jacobi
   ```
...
2. **Build carts if any changes were made:**
   ...
   ```bash
      carts cgeist jacobi.c -DMINI_DATASET -O0 --print-debug-info -S --raise-scf-to-affine -I. -I../common -I../utilities &> jacobi_seq.mlir
      carts run jacobi_seq.mlir --collect-metadata &> jacobi_arts_metadata.mlir
      carts cgeist jacobi.c -DMINI_DATASET -O0 --print-debug-info -S -fopenmp --raise-scf-to-affine -I. -I../common -I../utilities &> jacobi.mlir
   ```
...
3. **Run the pipeline and stop after any stage**
   ...
   For example, lets analyze the create-dbs pipeline
    ```bash
      carts run jacobi.mlir --create-dbs &> jacobi_create-dbs.mlir
    ```
...
```bash
    carts execute jacobi.c -O3 -DMINI_DATASET -I. -I../common -I../utilities
   ./jacobi_arts
```
...
The jacobi benchmark demonstrates a classic 5-point stencil pattern where each compute task depends on neighboring rows. This section walks through how CARTS handles out-of-bounds stencil indices.
...
```bash
carts run jacobi.mlir --db-opt --debug-only=db 2>&1 | grep -E "Stencil|bounds"
```
...
```bash
carts run jacobi.mlir --db-opt &> jacobi_db-opt.mlir
```
...
```bash
carts run jacobi.mlir --edt-lowering &> jacobi_edt-lowering.mlir
```
...
```bash
carts run jacobi.mlir --arts-to-llvm &> jacobi_arts-to-llvm.mlir
```
...
scf.for %slot = %c0 to %numDeps step %c1 {
   ...
   scf.if %is_valid {
      ...
      func.call @artsRecordDep(%dbGuid, %edtGuid, %slot, %mode)
   } else {
...
```bash
carts execute jacobi.c -O3 -DSIZE=100
./jacobi_arts
```
