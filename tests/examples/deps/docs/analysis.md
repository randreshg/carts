# Dependencies example analysis

Walk through these steps and fix any problem that you find in the way.

1. **Navigate to the deps example directory:**

   ```bash
   cd ~/Documents/carts/tests/examples/deps
   ```
...
3. **Create DBs with control DBs (expect coarse allocations + partition hints):**
   ...
   ```bash
   carts run deps.mlir --create-dbs --debug-only=db &> deps_create_dbs.mlir
   ```
