...
// RUN: %FileCheck %s --implicit-check-not='<coarse>' --implicit-check-not='partitioning(<coarse>)' < %t.dir/jacobi-for.pre-lowering.mlir
...
// Final host verification reads the same block DB directly.
// CHECK: arts_rt.db_gep(%[[UNEW_PTR]] : memref<?x?x!llvm.ptr>)
// CHECK: llvm.load
