...
// RT: %[[PAYLOAD_EPOCH:[A-Za-z0-9_]+]] = arts_rt.create_epoch
// RT: arts_rt.wait_on_epoch %[[PAYLOAD_EPOCH]] : i64
...
// RT: %[[HALO_EDT:[A-Za-z0-9_]+]] = arts_rt.edt_create(%{{[A-Za-z0-9_]+}} : memref<?xi64>){{.*}}{arts.create_id = {{[0-9]+}}
// RT: arts_rt.rec_dep %[[HALO_EDT]]
// RT-SAME: byte_offsets(%c0, %c0, %c7936, %c0, %c0, %c0, %c0)
// RT-SAME: byte_sizes(%c0, %c0, %c256, %c256, %c0, %c0, %c0)
// RT-SAME: acquire_modes = array<i32: 2, 1, 1, 1, 1, 1, 1>
// RT-SAME: dep_flags = array<i32: 0, 0, 4, 4, 0, 0, 0>
// RT: %[[VERIFY_EPOCH:[A-Za-z0-9_]+]] = arts_rt.create_epoch
// RT: %[[VERIFY_PACK:[A-Za-z0-9_]+]] = arts_rt.edt_param_pack
// RT: %[[VERIFY_EDT:[A-Za-z0-9_]+]] = arts_rt.edt_create(%[[VERIFY_PACK]] : memref<?xi64>){{.*}}epoch(%[[VERIFY_EPOCH]] : i64)
