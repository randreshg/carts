...
// RUN: %FileCheck %s --implicit-check-not='element_sizes[%c32, %c32]{{.*}}haloViewDependency' \
// RUN:   < %t.dir/jacobi-for.post-db-refinement.mlir

// ARTS represents 2D unit halos as explicit face dependencies. Row faces are
...
// CHECK-DAG: arts.db_alloc{{.*}}elementSizes[%c32, %c1]{{.*}}compact_halo_payload
// CHECK-DAG: arts.db_alloc{{.*}}elementSizes[%c32, %c1]{{.*}}compact_halo_payload
...
// CHECK: arts.db_acquire[<in>] {{.*}} element_offsets[%c31, %c0] element_sizes[%c1, %c32]
// CHECK-SAME: haloViewDependency
// CHECK-SAME: stencil_max_offsets = [0, 0]
// CHECK-SAME: stencil_min_offsets = [-1, 0]
// CHECK: arts.db_acquire[<in>] {{.*}} element_offsets[%c0, %c0] element_sizes[%c1, %c32]
// CHECK-SAME: haloViewDependency
