...
#pragma omp single
    {
      for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
#pragma omp task depend(out : A[i * N + j]) depend(in : A[(i - 1) * N + j])    \
    depend(in : A[i * N + (j - 1)]) firstprivate(i, j)
          {
...
#pragma omp taskwait
  ...
  /* A[i][j] = C(i+j, i) which is binomial coefficient, but simpler: each cell =
   * top+left+1 */
