...
- enough useful work per task to amortize launch;
...
- physical storage family count and dependency-window count.

The selected tile should be the same shape SDE stamps as `physicalBlockShape`,
avoiding a mismatch between loop tiling and storage materialization.
SDE should not create target runtime queries or worker-specific control; Core
...
- physical output/vector storage layout when the final result is array-backed;
...
The physical plan should stamp slab storage and bounded halo windows so Core
does not require full component tensors for every worker.
...
### Physical Storage Plan Stamping
...
All physical storage creation remains write-backed first. Reader-only input
tiling must be separately proved and cost-justified.
...
- Effect summaries that miss aliasing can make fusion or barrier removal
  unsound.
- Over-partitioning reader-only inputs can increase storage-object count and
