...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=canonicalize-memrefs
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=initial-cleanup
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=openmp-to-arts
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=openmp-to-arts --debug-only=convert_openmp_to_arts 2>&1
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=edt-transforms
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=edt-transforms --debug-only=edt 2>&1
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=loop-reordering
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=loop-reordering --debug-only=loop_reordering 2>&1
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=create-dbs
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=create-dbs --debug-only=create_dbs 2>&1
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=db-opt
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=db-opt --debug-only=db 2>&1
```
...
**Run Command:**
```bash
carts run <file>.mlir --stop-at=edt-opt
```

**Debug Command:**
```bash
carts run <file>.mlir --stop-at=edt-opt --debug-only=edt,arts_loop_fusion 2>&1
```
...
**Debug Command:**
```bash
carts run <file>.mlir --stop-at=concurrency --debug-only=concurrency,for_lowering 2>&1
```
...
carts run gemm.mlir --stop-at=loop-reordering --debug-only=loop_reordering 2>&1 | tee debug.log
...
carts run example.mlir --stop-at=canonicalize-memrefs -o stages/01_canonicalize.mlir
carts run example.mlir --stop-at=openmp-to-arts -o stages/04_openmp_to_arts.mlir
carts run example.mlir --stop-at=create-dbs -o stages/07_create_dbs.mlir
carts run example.mlir --stop-at=concurrency-opt -o stages/11_concurrency_opt.mlir

# Or dump all stages at once
carts run example.mlir --all-stages -o stages/

# Step 5: Debug specific pass if needed
carts run example.mlir --stop-at=create-dbs --debug-only=create_dbs 2>&1 | tee debug_createdb.log
