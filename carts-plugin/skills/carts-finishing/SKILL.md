...
- ARTS commits isolated codelet graph structure and preserves SDE-authored
  movement facts.
...
| 5 (#6) | targeted-fix | Keep SDE layout facts target-neutral and real. Add or adjust ARTS realization only when SDE have already committed valid layout and movement structure; ARTS consumes that structure plus abstract-machine topology without redetecting source patterns or pushing owner policy into ARTS-RT. |
...
5. **Apply the fix in the originating layer**, not the surface layer. SDE fixes
   layout/source facts, ARTS fixes collective/bridge materialization, ARTS
...
- `docs/compiler/dialects/*` — dialect ownership docs for SDE, ARTS, and ARTS-RT
