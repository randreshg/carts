...
8. **Multinode spot-check** (if the fix touches SDE distribution planning, DB refinement, ownership, EDT materialization, EpochLowering, or CPS logic). `dekk carts compile samples/<fixed-sample> -O3 -o .carts/outputs/regression/mn && ARTS_CONFIG=samples/arts_multinode.cfg .carts/outputs/regression/mn`. No regression vs prior multinode baseline.
