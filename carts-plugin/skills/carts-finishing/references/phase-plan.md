...
2. Add `tests/e2e/lit.local.cfg` rule for `arts_multinode.cfg`. Cover the 9 currently-passing samples (convolution, deps, jacobi/deps, matrix, matrixmul, parallel, parallel_for/{block,static}, task) under a multinode config (distribution is default-on).
