...
| Dependency | Communication and legality | explicit deps/params, storage views that consume committed owner dims, SDE movement representation, halo/contraction/reduction materialization, control-token boundaries |
...
- It must not become a benchmark-name shortcut for owner dims, owner maps,
  ...
  movement families come from SDE structure, and owner maps belong to ARTS
  realization.
...
- ARTS-RT lowers mechanically; it must not infer time bands, stencil legality,
  owner maps, movement families, or DB grain from runtime shape.
